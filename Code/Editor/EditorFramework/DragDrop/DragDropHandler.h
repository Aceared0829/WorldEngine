...
class WDragDropInfo;
class WDragDropConfig;

class W_EDITORFRAMEWORK_DLL WDragDropHandler : public WReflectedClass
{
  W_ADD_DYNAMIC_REFLECTION(WDragDropHandler, WReflectedClass);
...
public:
  ...
  /// Call this when a drag enter event occurs. Return value indicates whether a WDragDropHandler was found to handle the operation. If not,
  /// subsequent drag & drop updates are ignored.
  static bool BeginDragDropOperation(const WDragDropInfo* pInfo, WDragDropConfig* pConfigToFillOut = nullptr);

  /// Call this when a drag event occurs. Ignored if BeginDragDropOperation() was not successful.
  static void UpdateDragDropOperation(const WDragDropInfo* pInfo);

  /// Call this when a drop event occurs. Ignored if BeginDragDropOperation() was not successful.
  static void FinishDragDrop(const WDragDropInfo* pInfo);
...
protected:
  ...
  /// to take precedence.
  virtual float CanHandle(const WDragDropInfo* pInfo) const = 0;

  /// Potentially called by the drag drop target to request information about how to determine the WDragDropInfo data.
  virtual void RequestConfiguration(WDragDropConfig* pConfigToFillOut) {}

  /// Called shortly after CanHandle returned true to begin handling a drag operation.
  virtual void OnDragBegin(const WDragDropInfo* pInfo) = 0;

  /// Called to update the drag operation with the latest state.
  virtual void OnDragUpdate(const WDragDropInfo* pInfo) = 0;
  ...
  virtual void OnDrop(const WDragDropInfo* pInfo) = 0;

private:
  static WDragDropHandler* FindDragDropHandler(const WDragDropInfo* pInfo);

  static WDragDropHandler* s_pActiveDnD;
};
