...
function(W_check_build_type)
	# set the default build type
	if(NOT CMAKE_BUILD_TYPE)
		set(CMAKE_BUILD_TYPE ${W_BUILDTYPENAME_DEV} CACHE STRING "Choose the type of build, options are: None ${W_BUILDTYPENAME_DEBUG} ${W_BUILDTYPENAME_DEV} ${W_BUILDTYPENAME_RELEASE}." FORCE)
	endif()
endfunction()

# #####################################
# ## W_set_build_flags_msvc(<target>)
# #####################################
function(W_set_build_flags_msvc TARGET_NAME)
	...
	set(OPT_CPP_PRIVATE ${OPT_CPP_PRIVATE} "$<$<CONFIG:${W_BUILDTYPENAME_RELEASE_UPPER}>:/Zo>")
	set(OPT_CPP_PRIVATE ${OPT_CPP_PRIVATE} "$<$<CONFIG:${W_BUILDTYPENAME_DEV_UPPER}>:/Zo>")

	# /Ob1: Only consider functions for inlining that are marked with inline or forceinline
	set(OPT_CPP_PRIVATE ${OPT_CPP_PRIVATE} "$<$<CONFIG:${W_BUILDTYPENAME_DEBUG_UPPER}>:/Ob1>")

	# /Ox: favor speed for optimizations
	set(OPT_CPP_PRIVATE ${OPT_CPP_PRIVATE} "$<$<CONFIG:${W_BUILDTYPENAME_RELEASE_UPPER}>:/Ox>")

	# /Ob2: Consider all functions for inlining
	set(OPT_CPP_PRIVATE ${OPT_CPP_PRIVATE} "$<$<CONFIG:${W_BUILDTYPENAME_RELEASE_UPPER}>:/Ob2>")

	# /Oi: Replace some functions with intrinsics or other special forms of the function
	set(OPT_CPP_PRIVATE ${OPT_CPP_PRIVATE} "$<$<CONFIG:${W_BUILDTYPENAME_RELEASE_UPPER}>:/Oi>")

	if(W_CMAKE_ARCHITECTURE_X86)

		if(CMAKE_CXX_COMPILER_ID MATCHES "Clang")

			if(${W_MIN_REQUIRED_SSE_LEVEL} STREQUAL "SSE41")
			...
			if(${W_MIN_REQUIRED_SSE_LEVEL} STREQUAL "AVX")
	...
	if(${ARG_NO_DEBUG})
		...
		set_target_properties(${TARGET_NAME} PROPERTIES LINK_FLAGS_${W_BUILDTYPENAME_DEBUG_UPPER} ${LINKER_FLAGS_RELEASE})
		set_target_properties(${TARGET_NAME} PROPERTIES LINK_FLAGS_${W_BUILDTYPENAME_DEV_UPPER} ${LINKER_FLAGS_RELEASE})
	else()
		set_target_properties(${TARGET_NAME} PROPERTIES LINK_FLAGS_${W_BUILDTYPENAME_DEBUG_UPPER} ${LINKER_FLAGS_DEBUG})
		set_target_properties(${TARGET_NAME} PROPERTIES LINK_FLAGS_${W_BUILDTYPENAME_DEV_UPPER} ${LINKER_FLAGS_${W_DEV_BUILD_LINKERFLAGS}})
	endif()

	set_target_properties(${TARGET_NAME} PROPERTIES LINK_FLAGS_${W_BUILDTYPENAME_RELEASE_UPPER} ${LINKER_FLAGS_RELEASE})

	if(W_ENABLE_COMPILER_STATIC_ANALYSIS)
...
# #####################################
# ## W_set_build_flags_clang(<target>)
# #####################################
function(W_set_build_flags_clang TARGET_NAME)
	# Enable debug info
	...
	if(W_ENABLE_QT_SUPPORT)
		# Ignore any warnings caused by Qt headers
		target_compile_options(${TARGET_NAME} PRIVATE "--system-header-prefix=\"${W_QT_DIR}\"")
	endif()

	# Ignore any warnings caused by headers inside the ThirdParty directory.
	if(W_SUBMODULE_PREFIX_PATH)
		target_compile_options(${TARGET_NAME} PRIVATE "--system-header-prefix=\"${W_ROOT}/Code/ThirdParty\"")
	else()
		target_compile_options(${TARGET_NAME} PRIVATE "--system-header-prefix=\"${CMAKE_SOURCE_DIR}/Code/ThirdParty\"")
	endif()

	if(COMMAND W_platformhook_set_build_flags_clang)
		# call platform-specific hook
		W_platformhook_set_build_flags_clang(${TARGET_NAME})
	endif()
endfunction()

# #####################################
# ## W_set_build_flags_gcc(<target>)
# #####################################
function(W_set_build_flags_gcc TARGET_NAME)
	...
	if(NOT(CMAKE_CURRENT_SOURCE_DIR MATCHES "Code/ThirdParty"))
		# Warning / Error settings for W code
		# attributes = error if a attribute is placed incorrectly (e.g. W_FOUNDATION_DLL)
		# unused-result = error if [[nodiscard]] return value is not handeled (WResult)
...
# #####################################
# ## W_set_build_flags(<target>)
# #####################################
function(W_set_build_flags TARGET_NAME)
	...
	if(W_CMAKE_COMPILER_MSVC)
		W_set_build_flags_msvc(${TARGET_NAME} ${ARGN})
	endif()

	if(W_CMAKE_COMPILER_CLANG)
		W_set_build_flags_clang(${TARGET_NAME} ${ARGN})
	endif()

	if(W_CMAKE_COMPILER_GCC)
		W_set_build_flags_gcc(${TARGET_NAME} ${ARGN})
	endif()
endfunction()

# #####################################
# ## W_enable_strict_warnings(<target>)
# #####################################
function(W_enable_strict_warnings TARGET_NAME)
	if(W_CMAKE_COMPILER_MSVC)
