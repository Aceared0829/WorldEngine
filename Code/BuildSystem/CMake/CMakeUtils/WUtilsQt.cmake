...
set(W_ENABLE_QT_SUPPORT ON CACHE BOOL "Whether to add Qt support.")
set(W_QT_DIR $ENV{QTDIR} CACHE PATH "Directory of the Qt installation")

# #####################################
# ## W_requires_qt()
# #####################################
macro(W_requires_qt)
	W_requires(W_ENABLE_QT_SUPPORT)
endmacro()
...
macro(W_find_qt)
	...
	# WorldEngine requires at least Qt 6.3 because earlier versions have a bug which prevents the 3d viewport in the
	# Editor from working correctly.
	SET(W_REQUIRED_QT_VERSION "6.3")

	if(W_ENABLE_QT_SUPPORT)
		if(W_QT_DIR)
			find_package(Qt6 ${W_REQUIRED_QT_VERSION} COMPONENTS ${W_QT_COMPONENTS} OPTIONAL_COMPONENTS GuiPrivate REQUIRED PATHS ${W_QT_DIR})
		else()
			find_package(Qt6 ${W_REQUIRED_QT_VERSION} COMPONENTS ${W_QT_COMPONENTS} OPTIONAL_COMPONENTS GuiPrivate REQUIRED)
		endif()
	endif()
	...
	if (W_ENABLE_QT_SUPPORT)
		if (COMMAND W_platformhook_find_qt)
			W_platformhook_find_qt()
		endif()
	endif()
...
# ## W_prepare_find_qt()
# #####################################
function(W_prepare_find_qt)
	set(W_CACHED_QT_DIR "W_CACHED_QT_DIR-NOTFOUND" CACHE STRING "")
	mark_as_advanced(W_CACHED_QT_DIR FORCE)
	...
	if (W_ENABLE_QT_SUPPORT)
		...
		if (COMMAND W_platformhook_download_qt)
			W_platformhook_download_qt()
		endif()

		# # Download Qt package
		# #####################
		if(NOT "${W_QT_DIR}" STREQUAL "${W_CACHED_QT_DIR}")
			# Need to reset qt vars now so that 'find_package' is re-executed
			set(W_CACHED_QT_DIR ${W_QT_DIR} CACHE STRING "" FORCE)
		...
		if(W_QT_DIR)
			set(CMAKE_PREFIX_PATH ${W_QT_DIR} PARENT_SCOPE)
		endif()
	endif()
endfunction()

# #####################################
# ## W_link_target_qt(TARGET <target> COMPONENTS <qt components> [COPY_DLLS])
# #####################################
function(W_link_target_qt)
	W_pull_all_vars()
	...
	if(FN_ARG_UNPARSED_ARGUMENTS)
		message(FATAL_ERROR "W_link_target_qt: Invalid arguments '${FN_ARG_UNPARSED_ARGUMENTS}'")
	endif()
	...
	target_compile_definitions(${FN_ARG_TARGET} PUBLIC W_USE_QT)
...
# #####################################
# ## W_qt_wrap_target_moc_files(<target> <files>)
# #####################################
function(W_qt_wrap_target_moc_files TARGET_NAME FILES_TO_WRAP)
	W_requires_qt()
...
# #####################################
# ## W_qt_wrap_target_qrc_files(<target> <files>)
# #####################################
function(W_qt_wrap_target_qrc_files TARGET_NAME FILES_TO_WRAP)
	W_requires_qt()
...
# #####################################
# ## W_qt_wrap_target_files(<target> <files>)
# #####################################
function(W_qt_wrap_target_files TARGET_NAME FILES_TO_WRAP)
	W_requires_qt()

	W_qt_wrap_target_qrc_files(${TARGET_NAME} "${FILES_TO_WRAP}")

	W_qt_wrap_target_ui_files(${TARGET_NAME} "${FILES_TO_WRAP}")

	# in this automated method, we only MOC files that end with ".moc.h"
	list(FILTER FILES_TO_WRAP INCLUDE REGEX ".*\.moc\.h")
	W_qt_wrap_target_moc_files(${TARGET_NAME} "${FILES_TO_WRAP}")
endfunction()
