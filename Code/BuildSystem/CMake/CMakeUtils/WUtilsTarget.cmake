# #####################################
# ## W_create_target(<LIBRARY | APPLICATION> <target-name> [NO_PCH] [NO_UNITY] [NO_QT] [ALL_SYMBOLS_VISIBLE] [EXCLUDE_FOLDER_FOR_UNITY <relative-folder>...])
# #####################################

macro(W_create_target TYPE TARGET_NAME)
	W_apply_build_filter(${TARGET_NAME})

	set(ARG_OPTIONS NO_PCH NO_UNITY NO_QT NO_W_PREFIX ENABLE_RTTI NO_WARNINGS_AS_ERRORS NO_COMPLIANCE ALL_SYMBOLS_VISIBLE NO_DEBUG NO_EXPORT)
	...
	if(ARG_UNPARSED_ARGUMENTS)
		message(FATAL_ERROR "W_create_target: Invalid arguments '${ARG_UNPARSED_ARGUMENTS}'")
	endif()
	...
	if(DEFINED ARG_MANUAL_SOURCE_FILES)
		...
		W_glob_source_files(${CMAKE_CURRENT_SOURCE_DIR} ALL_SOURCE_FILES)
	endif()

	# SHARED_LIBRARY means always shared
	# LIBRARY means SHARED_LIBRARY when W_COMPILE_ENGINE_AS_DLL is on, otherwise STATIC_LIBRARY
	if((${TYPE} STREQUAL "LIBRARY") OR(${TYPE} STREQUAL "STATIC_LIBRARY") OR(${TYPE} STREQUAL "SHARED_LIBRARY"))
		if((${W_COMPILE_ENGINE_AS_DLL} AND(${TYPE} STREQUAL "LIBRARY")) OR(${TYPE} STREQUAL "SHARED_LIBRARY"))
		...
		if(ARG_NO_W_PREFIX)
			...
			W_add_output_W_prefix(${TARGET_NAME})
		endif()

		if(COMMAND W_platformhook_set_library_properties)
			W_platformhook_set_library_properties(${TARGET_NAME})
		endif()
	...
	elseif(${TYPE} STREQUAL "APPLICATION")
		...
		if(W_CMAKE_PLATFORM_ANDROID)
			# All W applications must include the native app glue implementation
		...
		if(COMMAND W_platformhook_set_application_properties)
			W_platformhook_set_application_properties(${TARGET_NAME})
		endif()

	else()
		message(FATAL_ERROR "W_create_target: Missing argument to specify target type. Pass in 'APP' or 'LIB'.")
	endif()
	...
	if(NOT ${ARG_NO_PCH})
		W_auto_pch(${TARGET_NAME} "${ALL_SOURCE_FILES}" ${ARG_EXCLUDE_FROM_PCH_REGEX})
	endif()
	...
	if((NOT ANDROID) OR(NOT W_CMAKE_INSIDE_VS))
		# PLATFORM-TODO
		W_set_default_target_output_dirs(${TARGET_NAME})
	endif()
	...
	if(${TYPE} STREQUAL "APPLICATION")
		W_android_add_default_content(${TARGET_NAME})
	endif()

	W_add_target_folder_as_include_dir(${TARGET_NAME} ${CMAKE_CURRENT_SOURCE_DIR})

	W_set_common_target_definitions(${TARGET_NAME})

	W_set_build_flags(${TARGET_NAME} ${ARGN})

	# On linux we want all symbols to be hidden by default. We manually "export" them.
	if(W_COMPILE_ENGINE_AS_DLL AND W_CMAKE_PLATFORM_LINUX AND NOT ARG_ALL_SYMBOLS_VISIBLE)
		# PLATFORM-TODO (use general hook as above?)
		target_compile_options(${TARGET_NAME} PRIVATE "-fvisibility=hidden")
	endif()

	W_set_project_ide_folder(${TARGET_NAME} ${CMAKE_CURRENT_SOURCE_DIR})

	# PLATFORM-TODO (use general hook as above?)
	# Pass the windows sdk include paths to the resource compiler when not generating a visual studio solution.
	if(W_CMAKE_PLATFORM_WINDOWS AND NOT W_CMAKE_GENERATOR_MSVC)
		set(RC_FILES ${ALL_SOURCE_FILES})
		list(FILTER RC_FILES INCLUDE REGEX ".*\\.rc$")

		if(RC_FILES)
			set_source_files_properties(${RC_FILES}
				PROPERTIES COMPILE_FLAGS "/I\"C:/Program Files (x86)/Windows Kits/10/Include/${W_CMAKE_WINDOWS_SDK_VERSION}/shared\" /I\"C:/Program Files (x86)/Windows Kits/10/Include/${W_CMAKE_WINDOWS_SDK_VERSION}/um\""
			)
		endif()
	endif()

	# add a platform specific include directory
	if (EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/Platform/${W_CMAKE_PLATFORM_POSTFIX}")
		target_include_directories(${TARGET_NAME} PUBLIC "${CMAKE_CURRENT_SOURCE_DIR}/Platform/${W_CMAKE_PLATFORM_POSTFIX}")
	endif()
	...
	if (W_CMAKE_PLATFORM_WEB)
		set_target_properties(${TARGET_NAME} PROPERTIES SUFFIX ".html")
	endif()

	if(NOT ${ARG_NO_QT})
		W_qt_wrap_target_files(${TARGET_NAME} "${ALL_SOURCE_FILES}")
	endif()

	W_ci_add_to_targets_list(${TARGET_NAME} C++)

	if(NOT ${ARG_NO_UNITY})
		W_generate_folder_unity_files_for_target(${TARGET_NAME} ${CMAKE_CURRENT_SOURCE_DIR} "${ARG_EXCLUDE_FOLDER_FOR_UNITY}")
	endif()
