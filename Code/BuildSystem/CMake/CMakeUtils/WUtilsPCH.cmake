...
# #####################################
# ## W_retrieve_target_pch(<target> <pch-name>)
# #####################################
function(W_retrieve_target_pch TARGET_NAME PCH_NAME)
...
# #####################################
# ## W_pch_use(<pch-header> <cpp-files>)
# #####################################
function(W_pch_use PCH_H TARGET_CPPS)
	if(NOT W_USE_PCH OR W_CMAKE_GENERATOR_MSVC)
...
# #####################################
# ## W_pch_create(<pch-header> <cpp-file>)
# #####################################
function(W_pch_create PCH_H TARGET_CPP)
	if(NOT W_CMAKE_GENERATOR_MSVC)
		return()
	endif()
...
# #####################################
# ## W_find_pch_in_file_list(<files> <out-pch-name>)
# #####################################
function(W_find_pch_in_file_list FILE_LIST PCH_PATH PCH_NAME)
...
# #####################################
# ## W_auto_pch(<target> <files> [<file-exclude-regex> ... ])
# #####################################
function(W_auto_pch TARGET_NAME FILES)
	foreach(EXCLUDE_PATTERN ${ARGN})
		list(FILTER FILES EXCLUDE REGEX ${EXCLUDE_PATTERN})
	endforeach()

	W_find_pch_in_file_list("${FILES}" PCH_PATH PCH_NAME)

	if(NOT PCH_NAME)
		return()
	endif()

	W_pch_create("${PCH_NAME}.h" ${PCH_PATH})

	W_pch_use("${PCH_NAME}.h" "${FILES}")

	W_set_target_pch(${TARGET_NAME} ${PCH_NAME})
endfunction()
