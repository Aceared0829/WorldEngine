#include <FoundationTest/FoundationTestPCH.h>

#include <Foundation/Configuration/Startup.h>
#include <Foundation/Types/UniquePtr.h>

namespace DequeTestDetail
{
  using st = WConstructionCounter;

  static WDeque<st> CreateArray(WUInt32 uiSize, WUInt32 uiOffset)
  {
    WDeque<st> a;
    a.SetCount(uiSize);

    for (WUInt32 i = 0; i < uiSize; ++i)
      a[i] = uiOffset + i;

    return a;
  }
} // namespace DequeTestDetail

W_CREATE_SIMPLE_TEST(Containers, Deque)
{
  W_TEST_BLOCK(WTestBlock::Enabled, "Fill / Empty (Sawtooth)")
  {
    WDeque<WInt32> d;

    WUInt32 uiVal = 0;

    for (WInt32 i = 0; i < 10000; ++i)
      d.PushBack(uiVal++);

    // this is kind of the worst case scenario, as it will deallocate and reallocate chunks in every loop
    // the smaller the chunk size, the more allocations will happen
    for (WUInt32 s2 = 0; s2 < 10; ++s2)
    {
      for (WInt32 i = 0; i < 1000; ++i)
        d.PopBack();
      for (WInt32 i = 0; i < 1000; ++i)
        d.PushBack(uiVal++);
    }
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "Fill / Empty")
  {
    WDeque<WInt32> d;

    W_TEST_BOOL(d.GetHeapMemoryUsage() == 0);

    for (WInt32 i = 0; i < 10000; ++i)
      d.PushBack(i);

    W_TEST_BOOL(d.GetHeapMemoryUsage() > 10000 * sizeof(WInt32));

    for (WInt32 i = 0; i < 10000; ++i)
      d.PopFront();
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "Queue Back")
  {
    WDeque<WInt32> d;

    d.PushBack(0);

    for (WInt32 i = 0; i < 10000; ++i)
    {
      d.PushBack(i);
      d.PopFront();
    }

    d.Compact();

    for (WInt32 i = 0; i < 10000; ++i)
    {
      d.PushBack(i);
      d.PopFront();
    }
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "Queue Front")
  {
    WDeque<WInt32> d;

    d.PushBack(0);

    for (WInt32 i = 0; i < 10000; ++i)
    {
      d.PushFront(i);
      d.PopBack();
    }

    d.Compact();

    for (WInt32 i = 0; i < 10000; ++i)
    {
      d.PushFront(i);
      d.PopBack();
    }
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "POD Types")
  {
    WDeque<WInt32> d1;
    d1.SetCount(5120);

    d1.Compact();

    W_TEST_BOOL(d1.GetCount() == 5120);


    d1.SetCount(1);
    d1.Compact();

    W_TEST_BOOL(d1.GetHeapMemoryUsage() > 0);

    d1.Clear();

    d1.Compact();

    W_TEST_BOOL(d1.GetHeapMemoryUsage() == 0);
  }

  WStartup::ShutdownCoreSystems();

  WStartup::StartupCoreSystems();

  W_TEST_BLOCK(WTestBlock::Enabled, "POD Types")
  {
    WDeque<WInt32> d1;
    d1.SetCount(1000);

    W_TEST_BOOL(d1.GetCount() == 1000);
    d1.Clear();
    W_TEST_BOOL(d1.IsEmpty());

    for (WInt32 i = 1; i < 1000; ++i)
    {
      d1.PushBack(i);

      W_TEST_BOOL(d1.PeekBack() == i);
      W_TEST_BOOL(d1.GetCount() == i);
      W_TEST_BOOL(!d1.IsEmpty());
    }

    d1.Clear();
    W_TEST_BOOL(d1.IsEmpty());

    for (WInt32 i = 1; i < 1000; ++i)
    {
      d1.PushFront(i);

      W_TEST_BOOL(d1.PeekFront() == i);
      W_TEST_BOOL(d1.GetCount() == i);
      W_TEST_BOOL(!d1.IsEmpty());
    }

    d1.Clear();
    W_TEST_BOOL(d1.IsEmpty());

    for (WInt32 i = 1; i <= 1000; ++i)
    {
      d1.PushFront(i);
      d1.PushBack(i);

      W_TEST_BOOL(d1.PeekFront() == i);
      W_TEST_BOOL(d1.PeekBack() == i);
      W_TEST_BOOL(d1.GetCount() == i * 2);
      W_TEST_BOOL(!d1.IsEmpty());
    }

    WDeque<WInt32> d2;
    d2 = d1;

    for (WInt32 i = 1000; i >= 1; --i)
    {
      W_TEST_BOOL(d1.PeekFront() == i);
      W_TEST_BOOL(d1.PeekBack() == i);
      W_TEST_BOOL(d1.GetCount() == i * 2);
      W_TEST_BOOL(!d1.IsEmpty());

      d1.PopFront();
      d1.PopBack();


      W_TEST_BOOL(d2.PeekFront() == i);
      W_TEST_BOOL(d2.PeekBack() == i);
      W_TEST_BOOL(d2.GetCount() == i * 2);
      W_TEST_BOOL(!d2.IsEmpty());

      d2.PopFront();
      d2.PopBack();
    }

    W_TEST_BLOCK(WTestBlock::Enabled, "STL Iterator")
    {
      WDeque<WInt32> a1;
      for (WInt32 i = 0; i < 1000; ++i)
        a1.PushBack(1000 - i - 1);

      // STL sort
      std::sort(begin(a1), end(a1));

      WUInt32 prev = 0;
      for (WUInt32 val : a1)
      {
        W_TEST_BOOL(prev <= val);
        prev = val;
      }

      // STL lower bound
      auto lb = std::lower_bound(begin(a1), end(a1), 400);
      W_TEST_BOOL(*lb == a1[400]);
    }

    W_TEST_BLOCK(WTestBlock::Enabled, "STL Reverse Iterator")
    {
      WDeque<WInt32> a1;
      for (WInt32 i = 0; i < 1000; ++i)
        a1.PushBack(1000 - i - 1);

      std::sort(rbegin(a1), rend(a1));

      // foreach
      WUInt32 prev = 1000;
      for (WUInt32 val : a1)
      {
        W_TEST_BOOL(prev >= val);
        prev = val;
      }

      // const array
      const WDeque<WInt32>& a2 = a1;

      // STL lower bound
      auto lb2 = std::lower_bound(rbegin(a2), rend(a2), 400);
      W_TEST_INT(*lb2, a2[1000 - 400 - 1]);
    }
  }

  WStartup::ShutdownCoreSystems();

  WStartup::StartupCoreSystems();

  W_TEST_BLOCK(WTestBlock::Enabled, "Non-POD Types")
  {
    W_TEST_BOOL(DequeTestDetail::st::HasAllDestructed());

    {
      WDeque<DequeTestDetail::st> v1;
      W_TEST_BOOL(DequeTestDetail::st::HasDone(0, 0));

      {
        v1.PushBack(DequeTestDetail::st(3));
        W_TEST_BOOL(DequeTestDetail::st::HasDone(2, 1));

        v1.PushBack();
        W_TEST_BOOL(DequeTestDetail::st::HasDone(1, 0));

        v1.PopBack();
        W_TEST_BOOL(DequeTestDetail::st::HasDone(0, 1));
      }
      {
        v1.PushFront(DequeTestDetail::st(3));
        W_TEST_BOOL(DequeTestDetail::st::HasDone(2, 1));

        v1.PushFront();
        W_TEST_BOOL(DequeTestDetail::st::HasDone(1, 0));

        v1.PopFront();
        W_TEST_BOOL(DequeTestDetail::st::HasDone(0, 1));
      }

      W_TEST_BOOL(v1.GetCount() == 2);

      v1.SetCount(12);
      W_TEST_BOOL(DequeTestDetail::st::HasDone(10, 0));

      {
        WDeque<DequeTestDetail::st> v2;
        W_TEST_BOOL(DequeTestDetail::st::HasDone(0, 0));

        v2 = v1;
        W_TEST_BOOL(DequeTestDetail::st::HasDone(12, 0));

        v2.Clear();
        W_TEST_BOOL(DequeTestDetail::st::HasDone(0, 12));

        WDeque<DequeTestDetail::st> v3(v1);
        W_TEST_BOOL(DequeTestDetail::st::HasDone(12, 0));

        WDeque<DequeTestDetail::st> v4(v1);
        W_TEST_BOOL(DequeTestDetail::st::HasDone(12, 0));

        v4.SetCount(0);
        W_TEST_BOOL(DequeTestDetail::st::HasDone(0, 12));
      }

      W_TEST_BOOL(DequeTestDetail::st::HasDone(0, 12));
    }

    W_TEST_BOOL(DequeTestDetail::st::HasAllDestructed());
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "SortingPrimitives")
  {
    WDeque<WUInt32> list;

    list.Sort();

    for (WUInt32 i = 0; i < 245; i++)
    {
      list.PushBack(std::rand());
    }
    list.Sort();

    WUInt32 last = 0;
    for (WUInt32 i = 0; i < list.GetCount(); i++)
    {
      W_TEST_BOOL(last <= list[i]);
      last = list[i];
    }
  }



  W_TEST_BLOCK(WTestBlock::Enabled, "Constructor")
  {
    WDeque<WInt32> a1;
    WDeque<DequeTestDetail::st> a2;

    W_TEST_BOOL(a1.GetCount() == 0);
    W_TEST_BOOL(a2.GetCount() == 0);
    W_TEST_BOOL(a1.IsEmpty());
    W_TEST_BOOL(a2.IsEmpty());
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "Copy Constructor")
  {
    WDeque<WInt32> a1;

    for (WInt32 i = 0; i < 32; ++i)
      a1.PushBack(rand() % 100000);

    WDeque<WInt32> a2 = a1;
    WDeque<WInt32> a3(a1);

    W_TEST_BOOL(a1.GetCount() == a2.GetCount());
    W_TEST_BOOL(a1.GetCount() == a3.GetCount());

    for (WUInt32 i = 0; i < a1.GetCount(); ++i)
    {
      W_TEST_BOOL(a1[i] == a2[i]);
      W_TEST_BOOL(a1[i] == a3[i]);
      W_TEST_BOOL(a2[i] == a3[i]);
    }
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "Move Constructor / Operator")
  {
    W_TEST_BOOL(DequeTestDetail::st::HasAllDestructed());

    {
      // move constructor
      WDeque<DequeTestDetail::st> a1(DequeTestDetail::CreateArray(100, 20));

      W_TEST_INT(a1.GetCount(), 100);
      for (WUInt32 i = 0; i < a1.GetCount(); ++i)
        W_TEST_INT(a1[i].m_iData, 20 + i);

      // move operator
      a1 = DequeTestDetail::CreateArray(200, 50);

      W_TEST_INT(a1.GetCount(), 200);
      for (WUInt32 i = 0; i < a1.GetCount(); ++i)
        W_TEST_INT(a1[i].m_iData, 50 + i);
    }

    W_TEST_BOOL(DequeTestDetail::st::HasAllDestructed());
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "operator = / operator == / operator !=")
  {
    WDeque<WInt32> a1;
    WDeque<WInt32> a2;

    W_TEST_BOOL(a1 == a2);

    for (WInt32 i = 0; i < 100; ++i)
      a1.PushBack(i);

    W_TEST_BOOL(a1 != a2);

    a2 = a1;

    W_TEST_BOOL(a1.GetCount() == a2.GetCount());

    for (WUInt32 i = 0; i < a1.GetCount(); ++i)
      W_TEST_BOOL(a1[i] == a2[i]);

    W_TEST_BOOL(a1 == a2);
    W_TEST_BOOL(a2 == a1);
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "Index operator")
  {
    WDeque<WInt32> a1;
    a1.SetCount(100);

    for (WInt32 i = 0; i < 100; ++i)
      a1[i] = i;

    for (WInt32 i = 0; i < 100; ++i)
      W_TEST_INT(a1[i], i);

    WDeque<WInt32> ca1;
    ca1 = a1;

    for (WInt32 i = 0; i < 100; ++i)
      W_TEST_INT(ca1[i], i);
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "SetCount / GetCount / IsEmpty")
  {
    WDeque<WInt32> a1;

    W_TEST_BOOL(a1.IsEmpty());

    for (WInt32 i = 0; i < 128; ++i)
    {
      a1.SetCount(i + 1);
      W_TEST_INT(a1[i], 0); // default init
      a1[i] = i;
      W_TEST_INT(a1[i], i);

      W_TEST_INT(a1.GetCount(), i + 1);
      W_TEST_BOOL(!a1.IsEmpty());
    }

    for (WInt32 i = 0; i < 128; ++i)
      W_TEST_INT(a1[i], i);

    for (WInt32 i = 128; i >= 0; --i)
    {
      a1.SetCount(i);

      W_TEST_INT(a1.GetCount(), i);

      for (WInt32 i2 = 0; i2 < i; ++i2)
        W_TEST_INT(a1[i2], i2);
    }

    W_TEST_BOOL(a1.IsEmpty());
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "SetCountUninitialized")
  {
    WDeque<WInt32> a1;

    W_TEST_BOOL(a1.IsEmpty());

    for (WInt32 i = 0; i < 128; ++i)
    {
      a1.SetCountUninitialized(i + 1);
      // no default init
      a1[i] = i;
      W_TEST_INT(a1[i], i);

      W_TEST_INT(a1.GetCount(), i + 1);
      W_TEST_BOOL(!a1.IsEmpty());
    }

    for (WInt32 i = 0; i < 128; ++i)
      W_TEST_INT(a1[i], i);

    for (WInt32 i = 128; i >= 0; --i)
    {
      a1.SetCountUninitialized(i);

      W_TEST_INT(a1.GetCount(), i);

      for (WInt32 i2 = 0; i2 < i; ++i2)
        W_TEST_INT(a1[i2], i2);
    }

    W_TEST_BOOL(a1.IsEmpty());
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "EnsureCount")
  {
    WDeque<WInt32> a1;

    W_TEST_INT(a1.GetCount(), 0);

    a1.EnsureCount(0);
    W_TEST_INT(a1.GetCount(), 0);

    a1.EnsureCount(1);
    W_TEST_INT(a1.GetCount(), 1);

    a1.EnsureCount(2);
    W_TEST_INT(a1.GetCount(), 2);

    a1.EnsureCount(1);
    W_TEST_INT(a1.GetCount(), 2);
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "Clear")
  {
    WDeque<WInt32> a1;
    a1.Clear();

    a1.PushBack(3);
    a1.Clear();

    W_TEST_BOOL(a1.IsEmpty());
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "Contains / IndexOf / LastIndexOf")
  {
    WDeque<WInt32> a1;

    for (WInt32 i = -100; i < 100; ++i)
      W_TEST_BOOL(!a1.Contains(i));

    for (WInt32 i = 0; i < 100; ++i)
      a1.PushBack(i);

    for (WInt32 i = 0; i < 100; ++i)
    {
      W_TEST_BOOL(a1.Contains(i));
      W_TEST_INT(a1.IndexOf(i), i);
      W_TEST_INT(a1.LastIndexOf(i), i);
    }
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "Insert")
  {
    WDeque<WInt32> a1;

    // always inserts at the front
    for (WInt32 i = 0; i < 100; ++i)
      a1.InsertAt(0, i);

    for (WInt32 i = 0; i < 100; ++i)
      W_TEST_INT(a1[i], 99 - i);
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "RemoveAndCopy")
  {
    WDeque<WInt32> a1;

    for (WInt32 i = 0; i < 100; ++i)
      a1.PushBack(i % 2);

    while (a1.RemoveAndCopy(1))
    {
    }

    W_TEST_BOOL(a1.GetCount() == 50);

    for (WUInt32 i = 0; i < a1.GetCount(); ++i)
      W_TEST_INT(a1[i], 0);
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "RemoveAndSwap")
  {
    WDeque<WInt32> a1;

    for (WInt32 i = 0; i < 10; ++i)
      a1.InsertAt(i, i); // inserts at the end

    a1.RemoveAndSwap(9);
    a1.RemoveAndSwap(7);
    a1.RemoveAndSwap(5);
    a1.RemoveAndSwap(3);
    a1.RemoveAndSwap(1);

    W_TEST_INT(a1.GetCount(), 5);

    for (WInt32 i = 0; i < 5; ++i)
      W_TEST_BOOL(WMath::IsEven(a1[i]));
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "RemoveAtAndCopy")
  {
    WDeque<WInt32> a1;

    for (WInt32 i = 0; i < 10; ++i)
      a1.InsertAt(i, i); // inserts at the end

    a1.RemoveAtAndCopy(9);
    a1.RemoveAtAndCopy(7);
    a1.RemoveAtAndCopy(5);
    a1.RemoveAtAndCopy(3);
    a1.RemoveAtAndCopy(1);

    W_TEST_INT(a1.GetCount(), 5);

    for (WInt32 i = 0; i < 5; ++i)
      W_TEST_INT(a1[i], i * 2);
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "RemoveAtAndSwap")
  {
    WDeque<WInt32> a1;

    for (WInt32 i = 0; i < 10; ++i)
      a1.InsertAt(i, i); // inserts at the end

    a1.RemoveAtAndSwap(9);
    a1.RemoveAtAndSwap(7);
    a1.RemoveAtAndSwap(5);
    a1.RemoveAtAndSwap(3);
    a1.RemoveAtAndSwap(1);

    W_TEST_INT(a1.GetCount(), 5);

    for (WInt32 i = 0; i < 5; ++i)
      W_TEST_BOOL(WMath::IsEven(a1[i]));
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "ExpandAndGetRef")
  {
    WDeque<WInt32> a1;

    for (WInt32 i = 0; i < 20; ++i)
    {
      WInt32& intRef = a1.ExpandAndGetRef();
      intRef = i * 5;
    }


    W_TEST_BOOL(a1.GetCount() == 20);

    for (WInt32 i = 0; i < 20; ++i)
    {
      W_TEST_INT(a1[i], i * 5);
    }
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "PushBack / PopBack / PeekBack")
  {
    WDeque<WInt32> a1;

    for (WInt32 i = 0; i < 10; ++i)
    {
      a1.PushBack(i);
      W_TEST_INT(a1.PeekBack(), i);
    }

    for (WInt32 i = 9; i >= 0; --i)
    {
      W_TEST_INT(a1.PeekBack(), i);
      a1.PopBack();
    }

    a1.PushBack(23);
    a1.PushBack(2);
    a1.PushBack(3);

    a1.PopBack(2);
    W_TEST_INT(a1.PeekBack(), 23);
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "PushFront / PopFront / PeekFront")
  {
    WDeque<WInt32> a1;

    for (WInt32 i = 0; i < 10; ++i)
    {
      a1.PushFront(i);
      W_TEST_INT(a1.PeekFront(), i);
    }

    for (WInt32 i = 9; i >= 0; --i)
    {
      W_TEST_INT(a1.PeekFront(), i);
      a1.PopFront();
    }

    a1.PushFront(23);
    a1.PushFront(2);
    a1.PushFront(3);

    a1.PopFront(2);
    W_TEST_INT(a1.PeekFront(), 23);
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "Construction / Destruction")
  {
    {
      W_TEST_BOOL(DequeTestDetail::st::HasAllDestructed());

      WDeque<DequeTestDetail::st> a1;
      WDeque<DequeTestDetail::st> a2;

      W_TEST_BOOL(DequeTestDetail::st::HasDone(0, 0)); // nothing has been constructed / destructed in between
      W_TEST_BOOL(DequeTestDetail::st::HasAllDestructed());

      a1.PushBack(DequeTestDetail::st(1));
      W_TEST_BOOL(DequeTestDetail::st::HasDone(2, 1)); // one temporary, one final (copy constructed)

      a1.InsertAt(0, DequeTestDetail::st(2));
      W_TEST_BOOL(DequeTestDetail::st::HasDone(2, 1)); // one temporary, one final (copy constructed)

      a2 = a1;
      W_TEST_BOOL(DequeTestDetail::st::HasDone(2, 0)); // two copies

      a1.Clear();
      W_TEST_BOOL(DequeTestDetail::st::HasDone(0, 2));

      a1.PushBack(DequeTestDetail::st(3));
      a1.PushBack(DequeTestDetail::st(4));
      a1.PushBack(DequeTestDetail::st(5));
      a1.PushBack(DequeTestDetail::st(6));

      W_TEST_BOOL(DequeTestDetail::st::HasDone(8, 4)); // four temporaries

      a1.RemoveAndCopy(DequeTestDetail::st(3));
      W_TEST_BOOL(DequeTestDetail::st::HasDone(1, 2)); // one temporary, one destroyed

      a1.RemoveAndCopy(DequeTestDetail::st(3));
      W_TEST_BOOL(DequeTestDetail::st::HasDone(1, 1)); // one temporary, none destroyed

      a1.RemoveAtAndCopy(0);
      W_TEST_BOOL(DequeTestDetail::st::HasDone(0, 1)); // one destroyed

      a1.RemoveAtAndSwap(0);
      W_TEST_BOOL(DequeTestDetail::st::HasDone(0, 1)); // one destroyed
    }

    // tests the destructor of a2 and a1
    W_TEST_BOOL(DequeTestDetail::st::HasAllDestructed());
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "Reserve")
  {
    W_TEST_BOOL(DequeTestDetail::st::HasAllDestructed());

    WDeque<DequeTestDetail::st> a;

    W_TEST_BOOL(DequeTestDetail::st::HasDone(0, 0)); // nothing has been constructed / destructed in between
    W_TEST_BOOL(DequeTestDetail::st::HasAllDestructed());

    a.Reserve(100);

    W_TEST_BOOL(DequeTestDetail::st::HasDone(0, 0)); // nothing has been constructed / destructed in between
    W_TEST_BOOL(DequeTestDetail::st::HasAllDestructed());

    a.SetCount(10);
    W_TEST_BOOL(DequeTestDetail::st::HasDone(10, 0));

    a.Reserve(100);
    W_TEST_BOOL(DequeTestDetail::st::HasDone(0, 0));

    a.SetCount(100);
    W_TEST_BOOL(DequeTestDetail::st::HasDone(90, 0));

    a.Reserve(200);
    W_TEST_BOOL(DequeTestDetail::st::HasDone(0, 0)); // nothing had to be copied over

    a.SetCount(200);
    W_TEST_BOOL(DequeTestDetail::st::HasDone(100, 0));
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "Compact")
  {
    W_TEST_BOOL(DequeTestDetail::st::HasAllDestructed());

    WDeque<DequeTestDetail::st> a;

    W_TEST_BOOL(DequeTestDetail::st::HasDone(0, 0)); // nothing has been constructed / destructed in between
    W_TEST_BOOL(DequeTestDetail::st::HasAllDestructed());

    a.SetCount(100);
    W_TEST_BOOL(DequeTestDetail::st::HasDone(100, 0));

    a.SetCount(200);
    W_TEST_BOOL(DequeTestDetail::st::HasDone(100, 0));

    a.SetCount(10);
    W_TEST_BOOL(DequeTestDetail::st::HasDone(0, 190));

    // no reallocations and copying, if the memory is already available
    a.SetCount(200);
    W_TEST_BOOL(DequeTestDetail::st::HasDone(190, 0));

    a.SetCount(10);
    W_TEST_BOOL(DequeTestDetail::st::HasDone(0, 190));

    // now we remove the spare memory
    a.Compact();
    W_TEST_BOOL(DequeTestDetail::st::HasDone(0, 0));

    // this time the array needs to be relocated, and thus the already present elements need to be copied
    a.SetCount(200);
    W_TEST_BOOL(DequeTestDetail::st::HasDone(190, 0));

    // this does not deallocate memory
    a.Clear();
    W_TEST_BOOL(DequeTestDetail::st::HasDone(0, 200));

    a.SetCount(100);
    W_TEST_BOOL(DequeTestDetail::st::HasDone(100, 0));

    // therefore no object relocation
    a.SetCount(200);
    W_TEST_BOOL(DequeTestDetail::st::HasDone(100, 0));

    a.Clear();
    W_TEST_BOOL(DequeTestDetail::st::HasDone(0, 200));

    // this will deallocate ALL memory
    a.Compact();

    a.SetCount(100);
    W_TEST_BOOL(DequeTestDetail::st::HasDone(100, 0));

    // this time objects need to be relocated
    a.SetCount(200);
    W_TEST_BOOL(DequeTestDetail::st::HasDone(100, 0));
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "GetContiguousRange")
  {
    // deques allocate data in 4 KB chunks, so an integer deque will have 1024 ints per chunk

    WDeque<WInt32> d;

    for (WUInt32 i = 0; i < 100 * 1024; ++i)
      d.PushBack(i);

    WDynamicArray<WInt32> a;
    a.SetCountUninitialized(d.GetCount());

    WUInt32 uiArrayPos = 0;

    for (WUInt32 i = 0; i < 100; ++i)
    {
      const WUInt32 uiOffset = i * 1024 + i;

      const WUInt32 uiRange = d.GetContiguousRange(uiOffset);

      W_TEST_INT(uiRange, 1024 - i);

      WMemoryUtils::Copy(&a[uiArrayPos], &d[uiOffset], uiRange);

      uiArrayPos += uiRange;
    }

    a.SetCountUninitialized(uiArrayPos);

    uiArrayPos = 0;

    for (WUInt32 i = 0; i < 100; ++i)
    {
      const WUInt32 uiOffset = i * 1024 + i;
      const WUInt32 uiRange = 1024 - i;

      for (WUInt32 r = 0; r < uiRange; ++r)
      {
        W_TEST_INT(a[uiArrayPos], uiOffset + r);
        ++uiArrayPos;
      }
    }
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "Swap")
  {
    WDeque<WInt32> a1, a2;

    WInt32 content1[] = {1, 2, 3, 4};
    WInt32 content2[] = {5, 6, 7, 8, 9};
    for (WInt32 i : content1)
    {
      a1.PushBack(i);
    }
    for (WInt32 i : content2)
    {
      a2.PushBack(i);
    }

    WInt32* a1Ptr = &a1[0];
    WInt32* a2Ptr = &a2[0];

    a1.Swap(a2);

    // The pointers should be simply swapped
    W_TEST_BOOL(a2Ptr == &a1[0]);
    W_TEST_BOOL(a1Ptr == &a2[0]);

    W_TEST_INT(W_ARRAY_SIZE(content1), a2.GetCount());
    W_TEST_INT(W_ARRAY_SIZE(content2), a1.GetCount());

    // The data should be swapped
    for (int i = 0; i < W_ARRAY_SIZE(content1); ++i)
    {
      W_TEST_INT(content1[i], a2[i]);
    }
    for (int i = 0; i < W_ARRAY_SIZE(content2); ++i)
    {
      W_TEST_INT(content2[i], a1[i]);
    }
  }

  W_TEST_BLOCK(WTestBlock::Enabled, "Move PushBack / PushFront")
  {
    WDeque<WUniquePtr<WUInt32>> a1, a2;
    a1.PushBack(WUniquePtr<WUInt32>(W_DEFAULT_NEW(WUInt32, 1)));
    a1.PushBack(WUniquePtr<WUInt32>(W_DEFAULT_NEW(WUInt32, 2)));

    a2.PushFront(WUniquePtr<WUInt32>(W_DEFAULT_NEW(WUInt32, 3)));
    a2.PushFront(WUniquePtr<WUInt32>(W_DEFAULT_NEW(WUInt32, 4)));

    a1.Swap(a2);

    W_TEST_INT(*a1[0].Borrow(), 4);
    W_TEST_INT(*a1[1].Borrow(), 3);

    W_TEST_INT(*a2[0].Borrow(), 1);
    W_TEST_INT(*a2[1].Borrow(), 2);
  }
}
