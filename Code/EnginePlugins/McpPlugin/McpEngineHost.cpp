...
/// a default would have every game and every WEditorProcessor child open a port that nobody asked for.
/// The server only exists when it was asked for by name.
static WCommandLineOptionInt s_opt_McpPort("_Mcp", "-mcpport",
...
static WCommandLineOptionInt s_opt_EditorMcpPort("_Mcp", "-editor-mcpport",
...
WMcpServer* WMcpEngineHost::s_pServer = nullptr;
WUInt64 WMcpEngineHost::s_uiFrameCount = 0;
bool WMcpEngineHost::s_bQuitRequested = false;

WUInt16 WMcpEngineHost::ResolvePort()
{
  const WInt32 iOwnPort = s_opt_McpPort.GetOptionValue(WCommandLineOption::LogMode::FirstTimeIfSpecified);

  if (iOwnPort > 0)
    return static_cast<WUInt16>(iOwnPort);

  const WInt32 iEditorPort = s_opt_EditorMcpPort.GetOptionValue(WCommandLineOption::LogMode::FirstTimeIfSpecified);
  ...
  if (iEditorPort > 0 && iEditorPort < 0xFFFF)
    return static_cast<WUInt16>(iEditorPort + 1);

  return 0;
}

void WMcpEngineHost::Startup()
{
  const WUInt16 uiPort = ResolvePort();
  ...
  if (pApp == nullptr)
  {
    WLog::Warning("MCP: no WGameApplicationBase, so there is no frame loop to answer requests from. Not starting the server.");
    return;
  }

  s_pServer = W_DEFAULT_NEW(WMcpServer, "WorldEngine");
  ...
  if (s_pServer->Start(uiPort).Failed())
  {
    ...
    WLog::Error("MCP: could not listen on port {}. The game runs without an MCP server.", uiPort);

    W_DEFAULT_DELETE(s_pServer);
    return;
  }

  pApp->m_ExecutionEvents.AddEventHandler(&WMcpEngineHost::ExecutionEventHandler);
  ...
  // engine process would simply never be answered. WPlayer free-runs and ignores this.
...
void WMcpEngineHost::Shutdown()
{
  ...
  {
    pApp->m_ExecutionEvents.RemoveEventHandler(&WMcpEngineHost::ExecutionEventHandler);
...
void WMcpEngineHost::RequestQuit()
{
  s_bQuitRequested = true;
}

void WMcpEngineHost::ExecutionEventHandler(const WGameApplicationExecutionEvent& e)
{
  if (e.m_Type == WGameApplicationExecutionEvent::Type::BeforeWorldUpdates)
  {
  ...
  if (e.m_Type == WGameApplicationExecutionEvent::Type::EndAppTick)
  {
    if (s_bQuitRequested)
    {
      ...
      WApplication::GetApplicationInstance()->QuitApplication();
    }
  }
}
