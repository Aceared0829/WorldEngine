...
/// Render data submitted by WTerrainPatchComponent.
///
/// Carries the handles and parameters needed by WTerrainHeightfieldRenderer to issue the draw call.
/// The vertex shader generates terrain geometry from SV_VertexID using the height buffer as input.
class W_TERRAINPLUGIN_DLL WTerrainHeightfieldRenderData : public WInstanceableRenderData
{
  W_ADD_DYNAMIC_REFLECTION(WTerrainHeightfieldRenderData, WInstanceableRenderData);
...
public:
  ...
  WGALBufferHandle m_hHeightBuffer;
  ...
  WGALBufferHandle m_hNormalBuffer;
  ...
  WGALBufferHandle m_hCellMaterialBuffer;

  /// Per-cell-corner 4-weight pack baked by Step3
  /// (uint per cell-corner, ((CellsPerSide+8)/WTerrainMaterialCellStep)²×4 entries).
  ...
  WGALBufferHandle m_hVertexWeightBuffer;

  /// One bit per stored-grid vertex, set when the vertex was carved away (bit i&31 of word i>>5).
  WGALBufferHandle m_hCarveMaskBuffer;

  /// Number of rendered quads per side (= WTerrainResolution enum value, e.g. 128).
  WUInt32 m_uiCellsPerSide = 128;
  ...
  WUInt32 m_uiDefaultMaterialIndex = 0;
...
class W_TERRAINPLUGIN_DLL WTerrainVoxelRenderData : public WInstanceableRenderData
{
  W_ADD_DYNAMIC_REFLECTION(WTerrainVoxelRenderData, WInstanceableRenderData);

public:
  WMaterialResourceHandle m_hMaterial;

  /// GPU vertex buffer (StructuredBuffer<VoxelGpuVertex>) written by surface nets shaders.
  WGALBufferHandle m_hGpuMeshVertices;

  /// GPU index buffer (StructuredBuffer<uint>) written by surface nets shaders.
  WGALBufferHandle m_hGpuMeshIndices;
  ...
  /// rendering path.
  WGALBufferHandle m_hGpuMeshDrawArgs;

  WUInt32 m_uiBaseMaterialIndex = 0;
};
