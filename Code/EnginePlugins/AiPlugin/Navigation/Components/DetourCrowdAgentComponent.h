#pragma once

#include <AiPlugin/AiPluginDLL.h>
#include <Core/World/Component.h>
#include <Core/World/World.h>
#include <Foundation/Containers/ArrayMap.h>

class WDetourCrowdAgentComponent;
struct dtCrowdAgentParams;
struct dtCrowdAgent;
class dtCrowd;

//////////////////////////////////////////////////////////////////////////

struct WDetourCrowdAgentRotationMode
{
  using StorageType = WUInt8;

  enum Enum
  {
    LookAtNextPathCorner,
    MatchVelocityDirection,

    Default = LookAtNextPathCorner
  };
};

W_DECLARE_REFLECTABLE_TYPE(W_AIPLUGIN_DLL, WDetourCrowdAgentRotationMode);

//////////////////////////////////////////////////////////////////////////

class W_AIPLUGIN_DLL WDetourCrowdAgentComponentManager : public WComponentManager<WDetourCrowdAgentComponent, WBlockStorageType::FreeList>
{
  using SUPER = WComponentManager<WDetourCrowdAgentComponent, WBlockStorageType::FreeList>;

public:
  explicit WDetourCrowdAgentComponentManager(WWorld* pWorld);
  ~WDetourCrowdAgentComponentManager();

  virtual void Initialize() override;

private:
  void AsyncUpdate(const UpdateContext& ctx);
  void SyncTransforms(const UpdateContext& ctx);

  static void FillDtCrowdAgentParams(const WDetourCrowdAgentComponent* pAgent, dtCrowdAgentParams& out_params);
  static dtCrowdAgent* TryGetValidDtAgent(dtCrowd* pDtCrowd, const WDetourCrowdAgentComponent* pAgent);

  WUInt32 m_uiNextUniqueId = 0;
  WArrayMap<WHashedString, dtCrowd*> m_CrowdPerNavMesh;
};

//////////////////////////////////////////////////////////////////////////

/// Implements navigation, path following and obstacle avoidance. Requires Recast navmesh.
///
/// This component provides the ability to navigate around the level (using Recast navmesh)
/// while avoiding other agents.
///
/// Call SetDestination() to command the agent to navigate to a position. Check HasDestination() to
/// determine whether the agent is still navigating. If pathfinding fails and AllowPartialPath was
/// false, m_uiSteeringFailedBit is set.
class W_AIPLUGIN_DLL WDetourCrowdAgentComponent : public WComponent
{
  W_DECLARE_COMPONENT_TYPE(WDetourCrowdAgentComponent, WComponent, WDetourCrowdAgentComponentManager);

  //////////////////////////////////////////////////////////////////////////
  // WComponent

protected:
  virtual void SerializeComponent(WWorldWriter& stream) const override;
  virtual void DeserializeComponent(WWorldReader& stream) override;

  //////////////////////////////////////////////////////////////////////////
  // WAgentSteeringComponent

public:
  WDetourCrowdAgentComponent();
  ~WDetourCrowdAgentComponent();

  /// Sets the position to navigate to.
  ///
  /// If the position is not on navmesh, the nearest point on navmesh (with some threshold)
  /// will be the actual target. You can get it by calling GetActualTargetPosition().
  void SetDestination(const WVec3& vGlobalPos, bool bAllowPartialPath);
  WVec3 GetDestination() const { return m_vDestination; }
  void CancelNavigation();
  bool HasDestination() const { return m_uiHasDestinationBit; }

  //////////////////////////////////////////////////////////////////////////
  // Properties
protected:
  float m_fRadius = 0.3f;                                      ///< [ property ]
  float m_fHeight = 1.8f;                                      ///< [ property ]
  float m_fMaxSpeed = 3.5f;                                    ///< [ property ]
  float m_fMaxAcceleration = 10.0f;                            ///< [ property ]
  float m_fStoppingDistance = 0.1f;                            ///< [ property ] If distance to the target is less than the stopping distance, the target is reached.
  WAngle m_MaxAngularSpeed = WAngle::MakeFromDegree(360.0f); ///< [ property ]
  WEnum<WDetourCrowdAgentRotationMode> m_RotationMode;       ///< [ property ]
  float m_fPushiness = 1.0f;                                   ///< [ property ] The agent will push other agents with lower pushiness and will get pushed by agents with higher pushiness.
  WHashedString m_sNavmeshConfig;                             ///< [ property ] Which navmesh to walk on.

public:
  float GetRadius() const { return m_fRadius; }
  float GetHeight() const { return m_fHeight; }
  float GetMaxSpeed() const { return m_fMaxSpeed; }
  float GetMaxAcceleration() const { return m_fMaxAcceleration; }
  float GetStoppingDistance() const { return m_fStoppingDistance; }
  WAngle GetMaxAngularSpeed() const { return m_MaxAngularSpeed; }
  /// While GetDestination() returns the requested target position,
  /// this one will return the actual point on navmesh that the agent is trying to reach.
  WVec3 GetActualDestination() const { return m_vActualDestination; }
  WDetourCrowdAgentRotationMode::Enum GetRotationMode() const { return m_RotationMode; }
  float GetPushiness() const { return m_fPushiness; }

  void SetRadius(float fRadius);
  void SetHeight(float fHeight);
  void SetMaxSpeed(float fMaxSpeed);
  void SetMaxAcceleration(float fMaxAcceleration);
  void SetStoppingDistance(float fStoppingDistance);
  void SetMaxAngularSpeed(WAngle maxAngularSpeed);
  void SetRotationMode(WDetourCrowdAgentRotationMode::Enum rotationMode) { m_RotationMode = rotationMode; }
  void SetPushiness(float fPushiness);

  WVec3 GetVelocity() const { return m_vVelocity; }

  //////////////////////////////////////////////////////////////////////////
  // Other
protected:
  WUInt8 m_uiDestinationChangedBit : 1;
  WUInt8 m_uiHasDestinationBit : 1;
  WUInt8 m_uiSteeringFailedBit : 1; ///< Set when pathfinding fails and AllowPartialPath is false.
  WUInt8 m_uiParamsChangedBit : 1;
  WUInt8 m_uiAllowPartialPathBit : 1;

  WUInt32 m_uiCrowdId = 0;
  WUInt32 m_uiAgentId = 0;
  WUInt32 m_uiUniqueAgentId = 0;
  WVec3 m_vVelocity = WVec3::MakeZero();
  WVec3 m_vDestination = WVec3::MakeZero();
  WVec3 m_vActualDestination = WVec3::MakeZero();
};
