...
WFileserveFileState WFileserveClientContext::GetFileStatus(WUInt16& inout_uiDataDirID, const char* szRequestedFile, FileStatus& inout_status,
  WDynamicArray<WUInt8>& out_fileContent, bool bForceThisDataDir) const
{
  for (WUInt16 i = static_cast<WUInt16>(m_MountedDataDirs.GetCount()); i > 0; --i)
  {
    ...
    WFileStats stat;
    if (WOSFile::GetFileStats(sAbsPath, stat).Failed())
      continue;

    inout_status.m_uiFileSize = stat.m_uiFileSize;

    const WInt64 iNewTimestamp = stat.m_LastModificationTime.GetInt64(WSIUnitOfTime::Microsecond);

    if (inout_status.m_iTimestamp == iNewTimestamp && inout_uiDataDirID == uiDataDirID)
      return WFileserveFileState::SameTimestamp;
    ...
    // read the entire file
    {
      ...
      WUInt64 uiNewHash = 1;
      out_fileContent.SetCountUninitialized((WUInt32)inout_status.m_uiFileSize);

      if (!out_fileContent.IsEmpty())
      {
        file.ReadBytes(out_fileContent.GetData(), out_fileContent.GetCount());
        uiNewHash = WHashingUtils::xxHash64(out_fileContent.GetData(), (size_t)out_fileContent.GetCount(), uiNewHash);
        ...
        if (inout_status.m_uiHash == uiNewHash && inout_uiDataDirID == uiDataDirID)
          return WFileserveFileState::SameHash;
      }
    ...
    return WFileserveFileState::Different;
  }
  ...
  if (inout_status.m_iTimestamp == 0 && inout_status.m_uiHash == 0)
    return WFileserveFileState::NonExistantEither;

  return WFileserveFileState::NonExistant;
}
