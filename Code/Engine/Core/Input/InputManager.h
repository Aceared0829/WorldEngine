...
struct W_CORE_DLL WInputActionConfig
{
  ...
  /// then that is an absolute movement and it does not depend on how much time elapsed. Therefore the WInputManager will take care NOT to
  ...
  /// exponential scaling (i.e SlotValue = WMath::Pow(SlotValue, -ScaleValue)). Default is 1.0f.
  ...
  WString m_sFilterByInputSlotX[MaxInputSlotAlternatives];
  ...
  WString m_sFilterByInputSlotY[MaxInputSlotAlternatives];
  ...
  enum OnLeaveArea
  {
    LoseFocus, ///< The input action will lose focus and thus get 'deactivated' immediately. Ie. it will return WKeyState::Released.
    KeepFocus, ///< The input action will keep focus and continue to return WKeyState::Down, until all trigger slots are actually released.
  };
  ...
  enum OnEnterArea
  {
    ActivateImmediately,     ///< The input action will immediately get activated and return WKeyState::Pressed, even though the input slots
...
///
/// The WInputManager is the central hub through which you can configure which keys will trigger which actions. You can query in which
...
class W_CORE_DLL WInputManager
{
public:
  ...
  static void Update(WTime timeDifference); // [tested]

  /// Changes the display name of an input slot.
  static void SetInputSlotDisplayName(WStringView sInputSlot, WStringView sDefaultDisplayName); // [tested]

  /// Returns the display name that was assigned to the given input slot.
  static WStringView GetInputSlotDisplayName(WStringView sInputSlot); // [tested]

  /// A shortcut to get the display name of the input slot bound to a given action
  ///
  /// If iTrigger is set, the name of that trigger (0 .. WInputActionConfig::MaxInputSlotAlternatives) will be used.
  ...
  static WStringView GetInputSlotDisplayName(WStringView sInputSet, WStringView sAction, WInt32 iTrigger = -1);
  ...
  /// a value of zero.
  static void SetInputSlotDeadZone(WStringView sInputSlot, float fDeadZone); // [tested]

  /// Returns the dead zone value for the given input slot.
  static float GetInputSlotDeadZone(WStringView sInputSlot); // [tested]

  /// Returns the flags for the given input slot.
  static WBitflags<WInputSlotFlags> GetInputSlotFlags(WStringView sInputSlot); // [tested]
  ...
  static WKeyState::Enum GetInputSlotState(WStringView sInputSlot, float* pValue = nullptr); // [tested]

  /// Returns an array that contains all the names of all currently known input slots.
  static void RetrieveAllKnownInputSlots(WDynamicArray<WStringView>& out_inputSlots);
  ...
  static WString RetrieveLastCharacters(bool bResetCurrent = true); // [tested]
  ...
  static void ClearInputMapping(WStringView sInputSet, WStringView sInputSlot); // [tested]
  ...
  ///   WInputManager do the frame time scaling, because it should not be applied to all input, e.g. mouse delta values should never be
  ...
  static void SetInputActionConfig(WStringView sInputSet, WStringView sAction, const WInputActionConfig& config, bool bClearPreviousInputMappings); // [tested]
  ...
  /// does not exist.
  static WInputActionConfig GetInputActionConfig(WStringView sInputSet, WStringView sAction); // [tested]

  /// Deletes all state associated with the given input action.
  ///
  /// It is not necessary to call this function for cleanup.
  static void RemoveInputAction(WStringView sInputSet, WStringView sAction); // [tested]
  ...
  static WKeyState::Enum GetInputActionState(WStringView sInputSet, WStringView sAction, float* pValue = nullptr, WInt8* pTriggeredSlot = nullptr); // [tested]

  /// Sets the display name for the given action.
  static void SetActionDisplayName(WStringView sAction, WStringView sDisplayName); // [tested]

  /// Returns the display name for the given action, or the action name itself, if no special display name was specified yet.
  static const WString GetActionDisplayName(WStringView sAction); // [tested]

  /// Returns the names of all currently registered input sets.
  static void GetAllInputSets(WDynamicArray<WString>& out_inputSetNames); // [tested]

  /// Returns the names of all input actions in the given input set.
  static void GetAllInputActions(WStringView sInputSetName, WDynamicArray<WString>& out_inputActions); // [tested]
  ...
  static void SetExclusiveInputSet(WStringView sExclusiveSet) { s_sExclusiveInputSet = sExclusiveSet; }

  /// Returns whether any input set gets input exclusively.
  static WStringView GetExclusiveInputSet() { return s_sExclusiveInputSet; }
  ...
  static void SetMouseCursor(const WMouseCursorDesc& desc);

  /// Returns the description of the custom mouse cursor that shall currently be displayed.
  static const WMouseCursorDesc& GetMouseCursor() { return s_MouseCursor; }

  /// Shortcut for SetMouseCursor(WMouseCursorDesc()), disables the custom mouse cursor.
  static void ClearMouseCursor();

  /// Incremented every time any value of the custom mouse cursor description changed.
  static WUInt32 GetMouseCursorChangeCounter() { return s_uiMouseCursorChangeCounter; }
  ...
  static WUInt32 GetHardwareCursorSize();

  /// Incremented only when WMouseCursorDesc::m_sCursor changed.
  ///
  ...
  static WUInt32 GetMouseCursorIdentifierChangeCounter() { return s_uiMouseCursorIdChangeCounter; }
  ...
  /// Prefer using WMouseCursorOverrideRequest instead.
  ///
  /// May only be called from the main thread.
  static WUInt32 PushMouseCursorOverride(const WMouseCursorOverrideDesc& desc);
  ...
  /// May only be called from the main thread.
  static void PopMouseCursorOverride(WUInt32 uiOverrideId);

  /// Returns the override that is currently in effect, ie. the one that was pushed last.
  ///
  /// If no override is registered, this returns WMouseCursorOverride::None with m_bForceNoClip
  /// set to false, ie. 'don't change anything', which is not the same as a default constructed
  /// WMouseCursorOverrideDesc.
  static WMouseCursorOverrideDesc GetActiveMouseCursorOverride();
  ...
  ///
  /// Note that when the input is injected after WInputManager::Update was called, its effect will be delayed by one frame.
  static void InjectInputSlotValue(WStringView sInputSlot, float fValue); // [tested]
  ...
  static WStringView GetPressedInputSlot(WInputSlotFlags::Enum mustHaveFlags, WInputSlotFlags::Enum mustNotHaveFlags); // [tested]

  /// Mostly for internal use. Converts a scan-code value to the string that is used inside the engine for that key.
  static WStringView ConvertScanCodeToEngineName(WUInt8 uiScanCode, bool bIsExtendedKey);

  /// Helper for retrieving the input slot string for touch point with a given index.
  static WStringView GetInputSlotTouchPoint(WUInt32 uiIndex);

  /// Helper for retrieving the input slot string for touch point x position with a given index.
  static WStringView GetInputSlotTouchPointPositionX(WUInt32 uiIndex);

  /// Helper for retrieving the input slot string for touch point y position with a given index.
  static WStringView GetInputSlotTouchPointPositionY(WUInt32 uiIndex);
  ...
  struct InputEventData
  {
    ...
    WStringView m_sInputSlot;
    WStringView m_sInputSet;
    WStringView m_sInputAction;
  };

  using WEventInput = WEvent<const InputEventData&>;

  /// Adds an event handler that is called for input events.
  static WEventSubscriptionID AddEventHandler(WEventInput::Handler handler) { return s_InputEvents.AddEventHandler(handler); }

  /// Removes a previously added event handler.
  static void RemoveEventHandler(WEventInput::Handler handler) { s_InputEvents.RemoveEventHandler(handler); }
  static void RemoveEventHandler(WEventSubscriptionID id) { s_InputEvents.RemoveEventHandler(id); }
  ...
  /// Fills the array with all WInputDevice instances of the given base type.
  template <typename TYPE>
  static void GetInputDevicesOfType(WDynamicArray<TYPE*>& out_devices)
  {
    out_devices.Clear();
  ...
  /// Fills the array with all WInputDevice instances of the given base type.
  static void GetInputDevicesOfType(const WRTTI* pRtti, WDynamicArray<WInputDevice*>& out_devices);

private:
  friend class WInputDevice;
  ...
  static void RegisterInputSlot(WStringView sName, WStringView sDefaultDisplayName, WBitflags<WInputSlotFlags> SlotFlags);
...
private:
  ...
  struct WInputSlot
  {
    ...
    WString m_sDisplayName;                  ///< The display name. Use this to present input slots in UIs.
    ...
    WKeyState::Enum m_State;                 ///< The current state.
    float m_fDeadZone;                        ///< The dead zone. Unless the value exceeds this, it reports a zero value.
    WBitflags<WInputSlotFlags> m_SlotFlags; ///< Describes the capabilities of the slot.
  };
  ...
  struct WActionData
  {
    ...
    WInputActionConfig m_Config; ///< The configuration that was specified by the user.

    float m_fValue;               ///< The current value. This is the maximum of all input slot values that trigger this action.
    WKeyState::Enum m_State;     ///< The current state. Derived from m_fValue.

    WInt8 m_iTriggeredViaAlternative;
  };

  using WActionMap = WMap<WString, WActionData>;    ///< Maps input action names to their data.
  using WInputSetMap = WMap<WString, WActionMap>;   ///< Maps input set names to their data.
  using WInputSlotsMap = WMap<WString, WInputSlot>; ///< Maps input slot names to their data.

  /// The internal data of the WInputManager. Not allocated until it is actually required.
  struct InternalData
  {
    WInputSetMap s_ActionMapping;                  ///< Maps input set names to their data.
    WInputSlotsMap s_InputSlots;                   ///< Maps input slot names to their data.
    WMap<WString, WString> s_ActionDisplayNames; ///< Stores a display name for each input action.
    WMap<WString, float> s_InjectedInputSlots;
  };

  /// Everything (Unicode) typed by the user since the last reset, as reported by the OS (on Windows: WM_CHAR).
  static WString s_sLastCharacters;

  static bool s_bInputSlotResetRequired;

  /// If not empty, all input for other input sets is returned as inactive ('key up')
  static WString s_sExclusiveInputSet;
  ...
  static WMouseCursorDesc s_MouseCursor;
  static WUInt32 s_uiMouseCursorChangeCounter;
  static WUInt32 s_uiMouseCursorIdChangeCounter;
  ...
  static WHybridArray<MouseCursorOverride, 4> s_MouseCursorOverrides;
  static WUInt32 s_uiNextMouseCursorOverrideId;
  ...
  static void UpdateInputActions(WTime tTimeDifference);

  /// Updates the state of all input actions in the given input set.
  static void UpdateInputActions(WStringView sInputSet, WActionMap& Actions, WTime tTimeDifference);
  ...
  static WActionMap::Iterator GetBestAction(WActionMap& Actions, const WString& sSlot, const WActionMap::Iterator& itFirst);

private:
  W_MAKE_SUBSYSTEM_STARTUP_FRIEND(Core, InputManager);

  static void DeallocateInternals();
  static InternalData& GetInternals();

  static WEventInput s_InputEvents;
