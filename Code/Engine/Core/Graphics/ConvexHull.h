...
class W_CORE_DLL WConvexHullGenerator
{
public:
  struct Face
  {
    ...
    WUInt16 m_uiVertexIdx[3];
  };

  WConvexHullGenerator();
  ~WConvexHullGenerator();
  ...
  void SetSimplificationMinTriangleAngle(WAngle angle) { m_MinTriangleAngle = angle; }
  ...
  /// Default is 5 degree.
  void SetSimplificationFlatVertexNormalThreshold(WAngle angle) { m_FlatVertexNormalThreshold = angle; }
  ...
  WResult Build(const WArrayPtr<const WVec3> vertices);

  /// When Build() was successful this can be called to retrieve the resulting vertices and triangles.
  void Retrieve(WDynamicArray<WVec3>& out_vertices, WDynamicArray<Face>& out_faces);

  /// Same as Retrieve() but only returns the vertices.
  void RetrieveVertices(WDynamicArray<WVec3>& out_vertices);

private:
  WResult ComputeCenterAndScale(const WArrayPtr<const WVec3> vertices);
  WResult StoreNormalizedVertices(const WArrayPtr<const WVec3> vertices);
  void StoreTriangle(WUInt16 i, WUInt16 j, WUInt16 k);
  WResult InitializeHull();
  WResult ComputeHull();
  bool IsInside(WUInt32 vtxId) const;
  void RemoveVisibleFaces(WUInt32 vtxId);
  void PatchHole(WUInt32 vtxId);
  ...
  WResult ProcessVertices(const WArrayPtr<const WVec3> vertices);

  struct TwoSet
  {
    W_ALWAYS_INLINE TwoSet()
    {
      a = 0xFFFF;
      b = 0xFFFF;
    }
    W_ALWAYS_INLINE void Add(WUInt16 x) { (a == 0xFFFF ? a : b) = x; }
    W_ALWAYS_INLINE bool Contains(WUInt16 x) { return a == x || b == x; }
    W_ALWAYS_INLINE void Remove(WUInt16 x) { (a == x ? a : b) = 0xFFFF; }
    W_ALWAYS_INLINE int GetSize() { return (a != 0xFFFF) + (b != 0xFFFF); }
  ...
  WAngle m_MinTriangleAngle = WAngle::MakeFromDegree(22.0f);
  WAngle m_FlatVertexNormalThreshold = WAngle::MakeFromDegree(5);
  ...
  WDynamicArray<WVec3d> m_Vertices;
  ...
  WDynamicArray<TwoSet> m_Edges;

  WDeque<Triangle> m_Triangles;
};
