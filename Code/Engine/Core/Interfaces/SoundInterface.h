...
/// Interface for sound system integration providing audio playback and control functionality.
///
...
class W_CORE_DLL WSoundInterface
{
public:
  ...
  virtual void LoadConfiguration(WStringView sFile) = 0;
  ...
  virtual void SetOverridePlatform(WStringView sPlatform) = 0;
  ...
  virtual void SetSoundGroupVolume(WStringView sVcaGroupGuid, float fVolume) = 0;
  virtual float GetSoundGroupVolume(WStringView sVcaGroupGuid) const = 0;

  /// Default is 1. Allows to set how many virtual listeners the sound is mixed for (split screen game play).
  virtual void SetNumListeners(WUInt8 uiNumListeners) = 0;
  virtual WUInt8 GetNumListeners() = 0;
  ...
  virtual void SetListener(WInt32 iIndex, const WVec3& vPosition, const WVec3& vForward, const WVec3& vUp, const WVec3& vVelocity) = 0;

  /// Plays a sound once. Callced by WSoundInterface::PlaySound().
  virtual WResult OneShotSound(WWorld* pWorld, WStringView sResourceID, const WTransform& globalPosition, float fPitch = 1.0f, float fVolume = 1.0f, bool bBlockIfNotLoaded = true) = 0;

  /// Plays a sound once.
  ///
  /// Convenience function to call OneShotSound() without having to retrieve the WSoundInterface first.
  ///
  ...
  /// If preloading is desired, you need to access the implementation-specific resource type directly (e.g. WFmodSoundEventResource).
  /// Also see WFmodSoundEventResource::PlayOnce().
  ...
  ///
  /// Also by default a pitch of 1 is always used. If the game speed is not 1 (WWorld clock), a custom pitch would need to be provided,
  /// if the sound should play at the same speed.
  static WResult PlaySound(WWorld* pWorld, WStringView sResourceID, const WTransform& globalPosition, float fPitch = 1.0f, float fVolume = 1.0f, bool bBlockIfNotLoaded = true);
};

/// Script extension class providing sound functionality for scripting environments.
class W_CORE_DLL WScriptExtensionClass_Sound
{
public:
  static void PlaySound(WWorld* pWorld, WStringView sResourceID, const WVec3& vGlobalPos, const WQuat& qGlobalRot, float fPitch = 1.0f, float fVolume = 1.0f, bool bBlockIfNotLoaded = true);
};

W_DECLARE_REFLECTABLE_TYPE(W_CORE_DLL, WScriptExtensionClass_Sound);
