...
/// These events may be sent by a specific WResource or by the WResourceManager
struct WResourceEvent
{
  enum class Type
  {
    ResourceExists,           ///< Used to broadcast that a resource exists. Used to inform inspection tools which resources are currently existing.
                              ///< Triggered by WResourceManager::BroadcastExistsEvent().
    ...
    ResourceContentUpdated,   ///< Sent shortly after WResource::UpdateContent() has been called on a resource.
...
/// Events sent by the WResourceManager
struct WResourceManagerEvent
{
  enum class Type
  {
    ManagerShuttingDown,      ///< Sent first thing by WResourceManager::OnEngineShutdown().
    ReloadAllResources,       ///< Sent by WResourceManager::ReloadAllResources() if any resource got unloaded (not yet reloaded)
  };

  Type m_Type;
};

/// The flags of an WResource instance.
struct WResourceFlags
{
  ...
  enum Enum
  {
    UpdateOnMainThread      = W_BIT(0),  ///< After loading the resource data on a thread, it must be uploaded on the main thread. Use this for resources which require a context that is only available on the main thread.
    NoFileAccessRequired    = W_BIT(1),  ///< The resource 'loading' does not require file accesses and can therefore be done on one or several non-file-loading threads. Use this for procedurally generated data.
    /// \todo implement NoFileAccessRequired
    ResourceHasFallback     = W_BIT(2),  ///< Specifies whether this resource has a valid fallback resource that could be used. Automatically updated in WResource::SetFallbackResource.
    ResourceHasTypeFallback = W_BIT(3),  ///< Specifies whether this resource has a valid type fallback that could be used.
    IsReloadable            = W_BIT(4),  ///< The resource was created, not loaded from file
    IsQueuedForLoading      = W_BIT(5),
    HasCustomDataLoader     = W_BIT(6),  ///< True if someone wants to update a resource with custom data and has created a resource loader to update this specific resource
    PreventFileReload       = W_BIT(7),  ///< Once this flag is set, no reloading from file is done, until the flag is manually removed. Automatically set when a custom loader is used. To restore a file to the disk state, this flag must be removed and then the resource can be reloaded.
    HasLowResData           = W_BIT(8),  ///< Whether low resolution data was set on a resource once before
    IsCreatedResource       = W_BIT(9),  ///< When this is set, the resource was created and not loaded from file
    Default                 = 0,
  };
...
W_DECLARE_FLAGS_OPERATORS(WResourceFlags);

/// Describes the state in which a resource can be in.
enum class WResourceState
{
...
struct WResourceLoadDesc
{
  WResourceState m_State = WResourceState::Invalid;
  ...
  WUInt8 m_uiQualityLevelsDiscardable = 0xFF;  // invalid
  ...
  WUInt8 m_uiQualityLevelsLoadable = 0xFF;     // invalid
};

/// Describes what data of a resource needs to be accessed and thus how much of the resource needs to be loaded.
///
/// \note Inspect the WResourceAcquireResult to know whether acquisition failed or whether a fallback was returned.
enum class WResourceAcquireMode
{
...
enum class WResourceAcquireResult
{
  None,             ///< No result available, ie the resource could not be loaded, not even a missing fallback was available, the resource pointer is
                    ///< nullptr. This result mode is only available when WResourceAcquireMode::BlockTillLoaded_NeverFail was used.
