...
class W_UTILITIES_DLL WDynamicOctree
{
...
public:
  ...
  /// purposes.
  void FindVisibleObjects(const WFrustum& viewfrustum, W_VISIBLE_OBJ_CALLBACK callback, void* pPassThrough) const;
...
private:
  ...
  bool InsertObject(const WVec3& vCenter, const WVec3& vHalfExtents, const WDynamicTree::WObjectData& Obj, float minx, float maxx, float miny,
    float maxy, float minz, float maxz, WUInt32 uiNodeID, WUInt32 uiAddID, WUInt32 uiSubAddID, WDynamicTreeObject* out_Object);

  /// Recursively checks which nodes are visible and calls the callback for each object at those nodes.
  void FindVisibleObjects(const WFrustum& Viewfrustum, W_VISIBLE_OBJ_CALLBACK Callback, void* pPassThrough, float minx, float maxx, float miny,
    float maxy, float minz, float maxz, WUInt32 uiNodeID, WUInt32 uiAddID, WUInt32 uiSubAddID, WUInt32 uiNextNodeID) const;

  /// Recursively checks in which node a point is located and calls the callback for all objects at those nodes.
  bool FindObjectsInRange(const WVec3& vPoint, W_VISIBLE_OBJ_CALLBACK Callback, void* pPassThrough, float minx, float maxx, float miny, float maxy,
