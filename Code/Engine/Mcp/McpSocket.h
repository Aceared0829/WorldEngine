...
/// The little bit of TCP that the MCP transport needs, with the platform differences taken out.
///
...
/// lives in WMcpTransport and WMcpServer.
///
/// Not copyable; a socket has an owner, and closing it twice closes a handle somebody else got given.
class W_MCP_DLL WMcpSocket
{
public:
  WMcpSocket();
  ~WMcpSocket();

  WMcpSocket(const WMcpSocket&) = delete;
  WMcpSocket& operator=(const WMcpSocket&) = delete;

  /// Binds to 127.0.0.1 at the given port and starts listening.
  ///
  ...
  WResult Listen(WUInt16 uiPort);

  /// The port that is actually bound, read back from the socket rather than from the argument.
  ...
  /// Waits up to \a timeout for an incoming connection.
  ///
  /// Returns W_FAILURE when nothing arrived in time, which is not an error - it is how the accept
  ...
  WResult Accept(WMcpSocket& out_client, WTime timeout);

  /// Reads whatever has arrived, blocking until something does.
  ///
  ...
  WInt32 Receive(void* pBuffer, WUInt32 uiSize);

  /// Writes the whole buffer, looping over partial writes.
  WResult SendAll(const void* pBuffer, WUInt32 uiSize);
