...
///
///  - The output goes into an WStringBuilder that the writer owns, so tools never deal with streams.
...
class W_MCP_DLL WMcpJsonWriter : public WStandardJSONWriter
{
public:
  WMcpJsonWriter();
  ~WMcpJsonWriter();
  ...
  virtual void WriteVariant(const WVariant& value) override;
