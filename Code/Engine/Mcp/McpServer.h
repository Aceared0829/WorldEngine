...
///
/// Hosts the JSON-RPC half; the sockets and the main-thread hand-off are WMcpTransport's job, and the
/// tools themselves come from WMcpToolRegistry. That split is what lets the editor and a game process
/// run the same server.
///
...
public:
  ...
  WMcpServer(WStringView sServerName);
  ~WMcpServer();

  /// Starts listening on 127.0.0.1 at the given port.
  WResult Start(WUInt16 uiPort);
  ...
  WUInt16 GetPort() const { return m_Transport.GetPort(); }
  ...
  static WMcpServer* GetInstance() { return s_pInstance; }

private:
  /// Turns one HTTP request into a response. Runs on the main thread.
  void HandleRequest(const WMcpHttpRequest& request, WMcpHttpResponse& ref_response);

  /// Turns one JSON-RPC message into a response body. Returns W_FAILURE for notifications,
  ...
  ///        meaningless and the caller must not send anything - see WMcpToolResult::m_bNotFinished.
  WResult HandleMessage(const WVariantDictionary& msg, WStringBuilder& out_sResponse, bool& out_bDeferred);

  WString m_sServerName;
  WMcpTransport m_Transport;

  static WMcpServer* s_pInstance;
};
