...
class W_FOUNDATION_DLL WRandom
{
public:
  WRandom();
  ~WRandom();
  ...
  void Initialize(WUInt64 uiSeed); // [tested]
  ...
  void Save(WStreamWriter& inout_stream) const; // [tested]

  /// Deserializes the current state
  void Load(WStreamReader& inout_stream); // [tested]
  ...
  WUInt32 UIntInRange(WUInt32 uiRange); // [tested]
  ...
  WUInt32 UInt32Index(WUInt32 uiArraySize, WUInt32 uiFallbackValue = WInvalidIndex);

  /// Returns a random uint16 in range [0 ; uiArraySize - 1].
  ///
  /// Same as UInt32Index() just for 16 bit indices.
  WUInt16 UInt16Index(WUInt16 uiArraySize, WUInt16 uiFallbackValue = 0xFFFF);

  /// Returns an int32 value in range [iMinValue ; iMaxValue]
  WInt32 IntMinMax(WInt32 iMinValue, WInt32 iMaxValue); // [tested]

  /// Returns a boolean either being true or false
  W_ALWAYS_INLINE bool Bool() { return static_cast<bool>(UInt() & 1); } // [tested]

  /// Returns a value in range [0.0 ; 1.0), ie. including zero, but excluding one
  W_ALWAYS_INLINE double DoubleZeroToOneExclusive() { return (double)UInt() / (double)(0xFFFFFFFFUL); } // [tested]

  /// Returns a value in range [0.0 ; 1.0], ie. including zero and one
  W_ALWAYS_INLINE double DoubleZeroToOneInclusive() { return (double)UInt() / (double)(0xFFFFFFFFUL + 1.0); } // [tested]
  ...
  /// Returns a value in range [0.0 ; 1.0), ie. including zero, but excluding one
  W_ALWAYS_INLINE float FloatZeroToOneExclusive() { return static_cast<float>(DoubleZeroToOneExclusive()); } // [tested]

  /// Returns a value in range [0.0 ; 1.0], ie. including zero and one
  W_ALWAYS_INLINE float FloatZeroToOneInclusive() { return static_cast<float>(DoubleZeroToOneInclusive()); } // [tested]

  /// Returns a float value in range [fMinValue ; fMaxValue]
  W_ALWAYS_INLINE float FloatMinMax(float fMinValue, float fMaxValue) { return static_cast<float>(DoubleMinMax(fMinValue, fMaxValue)); } // [tested]

  /// Returns a float value around fValue with a given variance (0 - 1 range)
  W_ALWAYS_INLINE float FloatVariance(float fValue, float fVariance) { return static_cast<float>(DoubleVariance(fValue, fVariance)); }

  /// Returns a float value between [-fAbsMaxValue; +fAbsMaxValue] with a Gaussian distribution.
  W_ALWAYS_INLINE float FloatVarianceAroundZero(float fAbsMaxValue) { return static_cast<float>(DoubleVarianceAroundZero(fAbsMaxValue)); }

private:
  WUInt32 m_uiIndex = 0;
  WUInt32 m_uiState[16];
};

W_DECLARE_REFLECTABLE_TYPE(W_FOUNDATION_DLL, WRandom);


/// A random number generator that produces values with a normal / Gaussian distribution
class W_FOUNDATION_DLL WRandomGauss
{
public:
  ...
  void Initialize(WUInt64 uiRandomSeed, WUInt32 uiMaxValue, float fVariance = 1.0f); // [tested]

  /// Returns a value in range [0; uiMaxValue - 1] with a Gaussian distribution. Ie. 0 is much more probable than uiMaxValue.
  WUInt32 UnsignedValue(); // [tested]
...
private:
  void SetupTable(WUInt32 uiMaxValue, float fSigma);

  float m_fSigma;
  double m_fAreaSum;
  WDynamicArray<float> m_GaussAreaSum;
  WRandom m_Generator;
};
