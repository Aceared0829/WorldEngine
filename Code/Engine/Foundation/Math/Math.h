...
namespace WMath
{
  ...
  /// Returns the linear interpolation of f1 and f2. factor is typically a value between 0 and 1. If factor is outside of that range, the result is a linear extrapolation.
  template <typename T>
  ...
  /// Evaluates the derivative of a cubic spline defined by four control points at time \a t and returns the interpolated result.
  /// Can be used with T as float, vec2, vec3 or vec4
  template <typename T, typename T2>
