...
/// A stream reader that will decompress data that was stored using the WCompressedStreamWriterZstd.
///
/// The reader takes another reader as its source for the compressed data (e.g. a file or a memory stream).
class W_FOUNDATION_DLL WCompressedStreamReaderZstd : public WStreamReader
{
public:
  WCompressedStreamReaderZstd(); // [tested]

  /// Takes an input stream as the source from which to read the compressed data.
  WCompressedStreamReaderZstd(WStreamReader* pInputStream); // [tested]
  ...
  void SetInputStream(WStreamReader* pInputStream); // [tested]
...
class W_FOUNDATION_DLL WCompressedStreamWriterZstd final : public WStreamWriter
{
public:
  ...
  WCompressedStreamWriterZstd();

  /// The constructor takes another stream writer to pass the output into, and a compression level.
  WCompressedStreamWriterZstd(WStreamWriter* pOutputStream, WUInt32 uiMaxNumWorkerThreads, Compression ratio = Compression::Default, WUInt32 uiCompressionCacheSizeKB = 4); // [tested]

  /// Calls FinishCompressedStream() internally.
  ~WCompressedStreamWriterZstd(); // [tested]
  ...
  void SetOutputStream(WStreamWriter* pOutputStream, WUInt32 uiMaxNumWorkerThreads, Compression ratio = Compression::Default, WUInt32 uiCompressionCacheSizeKB = 4); // [tested]
  ...
  WResult FinishCompressedStream(); // [tested]
  ...
  WUInt64 GetWrittenBytes() const { return m_uiWrittenBytes; } // [tested]
  ...
  virtual WResult Flush() override; // [tested]

private:
  WResult FlushWriteCache();

  WUInt64 m_uiUncompressedSize = 0;
  WUInt64 m_uiCompressedSize = 0;
  WUInt64 m_uiWrittenBytes = 0;
