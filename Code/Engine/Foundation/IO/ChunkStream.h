...
class W_FOUNDATION_DLL WChunkStreamWriter : public WStreamWriter
{
public:
  /// Pass the underlying stream writer to the constructor.
  WChunkStreamWriter(WStreamWriter& inout_stream); // [tested]
  ...
  /// and can be retrieved by WChunkStreamReader::BeginStream() for format compatibility checks.
  virtual void BeginStream(WUInt16 uiVersion); // [tested]
  ...
  virtual void BeginChunk(WStringView sName, WUInt32 uiVersion); // [tested]
...
private:
  ...
  WString m_sChunkName;
  WDeque<WUInt8> m_Storage;
  WStreamWriter& m_Stream;
};


/// Stream reader that parses chunked data written by WChunkStreamWriter
///
...
class W_FOUNDATION_DLL WChunkStreamReader : public WStreamReader
{
public:
  /// Pass the underlying stream writer to the constructor.
  WChunkStreamReader(WStreamReader& inout_stream); // [tested]
  ...
  ///
  /// Returns the version number that was passed to WChunkStreamWriter::BeginStream().
  /// Use this version for compatibility checking before processing chunks.
  virtual WUInt16 BeginStream(); // [tested]
  ...
  /// Metadata and status information for the current chunk
  struct ChunkInfo
  {
    ChunkInfo()
    {
    ...
    WString m_sChunkName;         ///< The name of the chunk.
    WUInt32 m_uiChunkVersion;     ///< The version number of the chunk.
    WUInt32 m_uiChunkBytes;       ///< The total size of the chunk.
    WUInt32 m_uiUnreadChunkBytes; ///< The number of bytes in the chunk that have not yet been read.
  };
