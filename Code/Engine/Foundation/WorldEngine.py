# LLDB Pretty Printers for WorldEngine
#
# Related files:
#   - WorldEngine.natvis  - Visual Studio / VS Code (MSVC) debugger visualizers
#   - WorldEngine-gdb.py  - GDB debugger visualizers
#
# To use with LLDB, add to your ~/.lldbinit:
#   command script import /path/to/WorldEngine/Code/Engine/Foundation/WorldEngine.py
...
def __lldb_init_module(debugger, internal_dict):
    ...
    # Strings
    debugger.HandleCommand('type synthetic add -x "^WHybridString<.+>$" --python-class WorldEngine.WHybridStringSynthProvider -w WorldEngine')
    debugger.HandleCommand('type summary add -x "^WHybridString<.+>$" --python-function WorldEngine.WHybridString_SummaryProvider -w WorldEngine')

    debugger.HandleCommand('type synthetic add WStringBuilder --python-class WorldEngine.WHybridStringSynthProvider -w WorldEngine')
    debugger.HandleCommand('type summary add WStringBuilder --python-function WorldEngine.WHybridString_SummaryProvider -w WorldEngine')

    debugger.HandleCommand('type synthetic add WStringView --python-class WorldEngine.WStringViewSynthProvider -w WorldEngine')
    debugger.HandleCommand('type summary add WStringView --python-function WorldEngine.WStringView_SummaryProvider -w WorldEngine')

    debugger.HandleCommand('type synthetic add WHashedString --python-class WorldEngine.WHashedStringSynthProvider -w WorldEngine')
    debugger.HandleCommand('type summary add WHashedString --python-function WorldEngine.WHashedString_SummaryProvider -w WorldEngine')

    debugger.HandleCommand('type summary add WStringIterator --python-function WorldEngine.WStringIterator_SummaryProvider -w WorldEngine')
    debugger.HandleCommand('type summary add WStringReverseIterator --python-function WorldEngine.WStringIterator_SummaryProvider -w WorldEngine')

    # Containers
    debugger.HandleCommand('type synthetic add -x "^WDynamicArray<.+>$" --python-class WorldEngine.WDynamicArrayBaseSynthProvider -w WorldEngine')
    debugger.HandleCommand('type synthetic add -x "^WHybridArray<.+>$" --python-class WorldEngine.WDynamicArrayBaseSynthProvider -w WorldEngine')
    debugger.HandleCommand('type synthetic add -x "^WSmallArray<.+>$" --python-class WorldEngine.WSmallArraySynthProvider -w WorldEngine')
    debugger.HandleCommand('type synthetic add -x "^WStaticArray<.+>$" --python-class WorldEngine.WStaticArraySynthProvider -w WorldEngine')
    debugger.HandleCommand('type synthetic add -x "^WStaticRingBuffer<.+>$" --python-class WorldEngine.WStaticRingBufferSynthProvider -w WorldEngine')

    debugger.HandleCommand('type synthetic add -x "^WArrayPtr<.+>$" --python-class WorldEngine.WArrayPtrSynthProvider -w WorldEngine')
    debugger.HandleCommand('type synthetic add WByteArrayPtr --python-class WorldEngine.WArrayPtrSynthProvider -w WorldEngine')
    debugger.HandleCommand('type synthetic add WConstByteArrayPtr --python-class WorldEngine.WArrayPtrSynthProvider -w WorldEngine')

    debugger.HandleCommand('type synthetic add -x "^WHashTable<.+>$" --python-class WorldEngine.WHashTableBaseSynthProvider -w WorldEngine')
    debugger.HandleCommand('type synthetic add -x "^WHashSet<.+>$" --python-class WorldEngine.WHashTableBaseSynthProvider -w WorldEngine')
    debugger.HandleCommand('type summary add -x "^WHashTableBase<.+>::Entry$" --summary-string "key = ${var.key}, value = ${var.value}" -w WorldEngine')

    debugger.HandleCommand('type synthetic add -x "^WMap<.+>$" --python-class WorldEngine.WMapSynthProvider -w WorldEngine')
    debugger.HandleCommand('type summary add -x "^WMapBase<.+>::Node$" --summary-string "key = ${var.m_Key}, value = ${var.m_Value}" -w WorldEngine')

    debugger.HandleCommand('type synthetic add -x "^WSet<.+>$" --python-class WorldEngine.WMapSynthProvider -w WorldEngine')
    debugger.HandleCommand('type summary add -x "^WSetBase<.+>::Node$" --summary-string "${var.m_Key}" -w WorldEngine')

    debugger.HandleCommand('type synthetic add -x "^WList<.+>$" --python-class WorldEngine.WListSynthProvider -w WorldEngine')

    debugger.HandleCommand('type synthetic add -x "^WDeque<.+>$" --python-class WorldEngine.WDequeSynthProvider -w WorldEngine')

    # Basic Types
    debugger.HandleCommand('type synthetic add -x "^WEnum<.+>$" --python-class WorldEngine.WEnumSynthProvider -w WorldEngine')
    debugger.HandleCommand('type summary add -x "^WEnum<.+>$" --python-function WorldEngine.WEnum_SummaryProvider -w WorldEngine')
    debugger.HandleCommand('type summary add -x "^WBitflags<.+>$" --python-function WorldEngine.WBitflags_SummaryProvider -w WorldEngine')
    debugger.HandleCommand('type summary add -x "^WVec2Template<.+>$" --summary-string "\{ x=${var.x}, y=${var.y} \}" -w WorldEngine')
    debugger.HandleCommand('type summary add -x "^WVec3Template<.+>$" --summary-string "\{ x=${var.x}, y=${var.y}, z=${var.z} \}" -w WorldEngine')
    debugger.HandleCommand('type summary add -x "^WVec4Template<.+>$" --summary-string "\{ x=${var.x}, y=${var.y}, z=${var.z}, w=${var.w} \}" -w WorldEngine')
    debugger.HandleCommand('type summary add -x "^WQuatTemplate<.+>$" --summary-string "\{ x=${var.x}, y=${var.y}, z=${var.z}, w=${var.w} \}" -w WorldEngine')
    debugger.HandleCommand('type summary add -x "^WPlaneTemplate<.+>$" --summary-string "\{ nx=${var.m_vNormal.x}, ny=${var.m_vNormal.y}, nz=${var.m_vNormal.z}, negDist=${var.m_fNegDistance} \}" -w WorldEngine')
    debugger.HandleCommand('type summary add "WColor" --summary-string "\{ r=${var.r}, g=${var.g}, b=${var.b}, a=${var.a} \}" -w WorldEngine')
    debugger.HandleCommand('type summary add "WColorGammaUB" --summary-string "\{ r=${var.r:d}, g=${var.g:d}, b=${var.b:d}, a=${var.a:d} \}" -w WorldEngine')
    debugger.HandleCommand('type summary add "WColorLinearUB" --summary-string "\{ r=${var.r:d}, g=${var.g:d}, b=${var.b:d}, a=${var.a:d} \}" -w WorldEngine')
    debugger.HandleCommand('type summary add "WTime" --summary-string "\{ seconds=${var.m_fTime} \}" -w WorldEngine')
    debugger.HandleCommand('type summary add "WAngle" --python-function WorldEngine.WAngle_SummaryProvider -w WorldEngine')
    debugger.HandleCommand('type summary add "WUuid" --python-function WorldEngine.WUuid_SummaryProvider -w WorldEngine')
    debugger.HandleCommand('type synthetic add "WMat3" --python-class WorldEngine.WMat3SynthProvider -w WorldEngine')
    debugger.HandleCommand('type synthetic add "WMat4" --python-class WorldEngine.WMat4SynthProvider -w WorldEngine')
    debugger.HandleCommand('type synthetic add "WVariant" --python-class WorldEngine.WVariantSynthProvider -w WorldEngine')
    debugger.HandleCommand('type summary add "WVariant" --python-function WorldEngine.WVariant_SummaryProvider -w WorldEngine')

    debugger.HandleCommand('type category enable WorldEngine')
...
class WHybridStringSynthProvider:
    ...
    def update(self):
        ...
        try:
            ...
            logger >> "WHybridStringSynthProvider.update: " + str(inst)
    ...
    def get_child_at_index(self, index):
        ...
        try:
            ...
            logger >> "WHybridStringSynthProvider.get_child_at_index: " + str(inst)
            return None

def WHybridString_SummaryProvider(valobj, dict):
    logger = lldb.formatters.Logger.Logger()
    try:
        ...
        logger >> "WHybridString_SummaryProvider: " + str(inst)
        return "<error>"

class WStringViewSynthProvider:
    def __init__(self, valobj, dict):
        logger = lldb.formatters.Logger.Logger()
        self.valobj = valobj
    ...
    def update(self):
        ...
        try:
            ...
            logger >> "WStringViewSynthProvider.update: " + str(inst)
    ...
    def get_child_at_index(self, index):
        ...
        try:
            ...
            logger >> "WStringViewSynthProvider.get_child_at_index: " + str(inst)
            return None

def WStringView_SummaryProvider(valobj, dict):
    logger = lldb.formatters.Logger.Logger()
    try:
        content = valobj.GetChildAtIndex(0)
        ...
        logger >> "WStringView_SummaryProvider: " + str(inst)
        return "<error>"

class WHashedStringSynthProvider:
    def __init__(self, valobj, dict):
        logger = lldb.formatters.Logger.Logger()
        self.valobj = valobj
    ...
    def update(self):
        ...
        try:
            ...
            logger >> "WHashedStringSynthProvider.update: " + str(inst)
    ...
    def get_child_at_index(self, index):
        ...
        try:
            ...
            logger >> "WHashedStringSynthProvider.get_child_at_index: " + str(inst)
            return None

def WHashedString_SummaryProvider(valobj, dict):
    logger = lldb.formatters.Logger.Logger()
    try:
        content = valobj.GetChildAtIndex(0)
        return WHybridString_SummaryProvider(content, dict)
    except Exception as inst:
        logger >> "WHashedString_SummaryProvider: " + str(inst)
        return "<error>"

def WStringIterator_SummaryProvider(valobj, dict):
    logger = lldb.formatters.Logger.Logger()
    try:
        ...
        logger >> "WStringIterator_SummaryProvider: " + str(inst)
        return "<error>"
...
class WDynamicArrayBaseSynthProvider(ArraySynthProvider):
    def __init__(self, valobj, dict):
    ...
    def update(self):
        ...
        try:
            ...
            logger >> "WDynamicArrayBaseSynthProvider.update: " + str(inst)
    ...
    def num_children(self):
        ...
        try:
            ...
            logger >> "WDynamicArrayBaseSynthProvider.num_children: " + str(inst)
            return 0
    ...
    def get_child_at_index(self, index):
        ...
        try:
            ...
            logger >> "WDynamicArrayBaseSynthProvider.get_child_at_index: " + str(inst)
            return None

class WSmallArraySynthProvider(ArraySynthProvider):
    def __init__(self, valobj, dict):
        super().__init__(valobj, dict)
    ...
    def update(self):
        ...
        try:
            ...
            logger >> "WSmallArraySynthProvider.update: " + str(inst)
    ...
    def num_children(self):
        ...
        try:
            ...
            logger >> "WSmallArraySynthProvider.num_children: " + str(inst)
            return 0
    ...
    def get_child_at_index(self, index):
        ...
        try:
            ...
            logger >> "WSmallArraySynthProvider.get_child_at_index: " + str(inst)
            return None

class WStaticArraySynthProvider(ArraySynthProvider):
    def __init__(self, valobj, dict):
        super().__init__(valobj, dict)
    ...
    def update(self):
        ...
        try:
            ...
            logger >> "WStaticArraySynthProvider.update: " + str(inst)
    ...
    def num_children(self):
        ...
        try:
            ...
            logger >> "WStaticArraySynthProvider.num_children: " + str(inst)
            return 0
    ...
    def get_child_at_index(self, index):
        ...
        try:
            ...
            logger >> "WStaticArraySynthProvider.get_child_at_index: " + str(inst)
            return None

class WStaticRingBufferSynthProvider(ArraySynthProvider):
    def __init__(self, valobj, dict):
        super().__init__(valobj, dict)
    ...
    def update(self):
        ...
        try:
            ...
            logger >> "WStaticRingBufferSynthProvider.update: " + str(inst)
    ...
    def get_child_at_index(self, index):
        ...
        try:
            ...
            logger >> "WStaticRingBufferSynthProvider.get_child_at_index: " + str(inst)
            return None

class WArrayPtrSynthProvider(ArraySynthProvider):
    def __init__(self, valobj, dict):
        super().__init__(valobj, dict)
    ...
    def update(self):
        ...
        try:
            ...
            logger >> "WArrayPtrSynthProvider.update: " + str(inst)
    ...
    def num_children(self):
        ...
        try:
            ...
            logger >> "WArrayPtrSynthProvider.num_children: " + str(inst)
            return 0
    ...
    def get_child_at_index(self, index):
        ...
        try:
            ...
            logger >> "WArrayPtrSynthProvider.get_child_at_index: " + str(inst)
            return None

class WHashTableBaseSynthProvider(ArraySynthProvider):
    def __init__(self, valobj, dict):
        super().__init__(valobj, dict)
    ...
    def update(self):
        ...
        try:
            ...
            logger >> "WHashTableBaseSynthProvider.update: " + str(inst)
    ...
    def num_children(self):
        ...
        try:
            ...
            logger >> "WHashTableBaseSynthProvider.num_children: " + str(inst)
            return 0
    ...
    def get_child_at_index(self, index):
        ...
        try:
            ...
            logger >> "WHashTableBaseSynthProvider.get_child_at_index: " + str(inst)
            return None

class WListSynthProvider:
    def __init__(self, valobj, dict):
        logger = lldb.formatters.Logger.Logger()
        self.valobj = valobj
    ...
    def update(self):
        ...
        try:
            ...
            logger >> "WListSynthProvider.update: " + str(inst)
    ...
    def num_children(self):
        ...
        try:
            ...
            logger >> "WListSynthProvider.num_children: " + str(inst)
            return 0
    ...
    def get_child_at_index(self, index):
        ...
        try:
            ...
            logger >> "WListSynthProvider.get_child_at_index: " + str(inst)
            return None

class WDequeSynthProvider(ArraySynthProvider):
    def __init__(self, valobj, dict):
        super().__init__(valobj, dict)
    ...
    def update(self):
        ...
        try:
            ...
            logger >> "WDequeSynthProvider.update: " + str(inst)
    ...
    def num_children(self):
        ...
        try:
            ...
            logger >> "WDequeSynthProvider.num_children: " + str(inst)
            return 0
    ...
    def get_child_at_index(self, index):
        ...
        try:
            ...
            logger >> "WDequeSynthProvider.get_child_at_index: " + str(inst)
            return None

class WMapSynthProvider:
    def __init__(self, valobj, dict):
        logger = lldb.formatters.Logger.Logger()
        self.valobj = valobj
        # reading the data through the debugger can take a long time
    ...
    def update(self):
        ...
        try:
            ...
            logger >> "WMapSynthProvider.update: " + str(inst)
    ...
    def num_children(self):
        ...
        try:
            ...
            logger >> "WMapSynthProvider.num_children: " + str(inst)
            return 0
    ...
    def get_child_at_index(self, index):
        ...
        try:
            ...
            logger >> "WMapSynthProvider.get_child_at_index: " + str(inst)
            return None
...
class WEnumSynthProvider:
    ...
    def update(self):
        ...
        try:
            ...
            logger >> "WEnumSynthProvider.update: " + str(inst)
        return False
    ...
    def get_child_at_index(self, index):
        ...
        try:
            ...
            logger >> "WEnumSynthProvider.get_child_at_index: " + str(inst)
            return None

def WEnum_SummaryProvider(valobj, dict):
    logger = lldb.formatters.Logger.Logger()
    try:
        content = valobj.GetChildAtIndex(0)
        return make_string(content)
    except Exception as inst:
        logger >> "WEnum_SummaryProvider: " + str(inst)
        return "<error>"

def WBitflags_SummaryProvider(valobj, dict):
    logger = lldb.formatters.Logger.Logger()
    try:
        ...
        logger >> "WBitflags_SummaryProvider: " + str(inst)
        return "<error>"
...
def WAngle_SummaryProvider(valobj, dict):
    logger = lldb.formatters.Logger.Logger()
    try:
        ...
        logger >> "WAngle_SummaryProvider: " + str(inst)
        return "<error>"

def WUuid_SummaryProvider(valobj, dict):
    logger = lldb.formatters.Logger.Logger()
    try:
        ...
        logger >> "WUuid_SummaryProvider: " + str(inst)
        return "<error>"

class WMat3SynthProvider:
    def __init__(self, valobj, dict):
        self.valobj = valobj
    ...
    def update(self):
        ...
        try:
            ...
            logger >> "WMat3SynthProvider.update: " + str(inst)
    ...
    def get_child_at_index(self, index):
        ...
        try:
            if index == 0:
                expr = f"(WVec3*)" +str(self.m_fElementsCM.GetAddress())
            ...
            logger >> "WMat3SynthProvider.get_child_at_index: " + str(inst)
            return None

class WMat4SynthProvider:
    def __init__(self, valobj, dict):
        self.valobj = valobj
    ...
    def update(self):
        ...
        try:
            ...
            logger >> "WMat4SynthProvider.update: " + str(inst)
    ...
    def get_child_at_index(self, index):
        ...
        try:
            if index == 0:
                expr = f"(WVec4*)" +str(self.m_fElementsCM.GetAddress())
                Column0 = self.valobj.CreateValueFromExpression('Column0', expr)
                return Column0
            if index == 1:
                address = self.m_fElementsCM.GetAddress()
                address.OffsetAddress(self.element_size * 4)
                expr = f"(WVec4*)" +str(address)
                Column1 = self.valobj.CreateValueFromExpression('Column1', expr)
                return Column1
            if index == 2:
                address = self.m_fElementsCM.GetAddress()
            ...
            logger >> "WMat4SynthProvider.get_child_at_index: " + str(inst)
            return None

class WVariantSynthProvider:
    ...
    def __init__(self, valobj, dict):
        self.valobj = valobj
        if not WVariantSynthProvider.variantMap:
            enum_type = self.valobj.GetTarget().FindTypes("WVariantType::Enum").GetTypeAtIndex(0)
            for v in enum_type.GetEnumMembers():
                bit_value = v.GetValueAsUnsigned()
                WVariantSynthProvider.variantMap[bit_value] = v.GetName()

            WVariantSynthProvider.variantTargetTypeMap["Bool"] = "bool"
            WVariantSynthProvider.variantTargetTypeMap["Int8"] = "WInt8"
            WVariantSynthProvider.variantTargetTypeMap["UInt8"] = "WUInt8"
            WVariantSynthProvider.variantTargetTypeMap["Int16"] = "WInt16"
            WVariantSynthProvider.variantTargetTypeMap["UInt16"] = "WUInt16"
            WVariantSynthProvider.variantTargetTypeMap["Int32"] = "WInt32"
            WVariantSynthProvider.variantTargetTypeMap["UInt32"] = "WUInt32"
            WVariantSynthProvider.variantTargetTypeMap["Int64"] = "WInt64"
            WVariantSynthProvider.variantTargetTypeMap["UInt64"] = "WUInt64"
            WVariantSynthProvider.variantTargetTypeMap["Float"] = "float"
            WVariantSynthProvider.variantTargetTypeMap["Double"] = "double"
            WVariantSynthProvider.variantTargetTypeMap["Color"] = "WColor"
            WVariantSynthProvider.variantTargetTypeMap["Vector2"] = "WVec2"
            WVariantSynthProvider.variantTargetTypeMap["Vector3"] = "WVec3"
            WVariantSynthProvider.variantTargetTypeMap["Vector4"] = "WVec4"
            WVariantSynthProvider.variantTargetTypeMap["Vector2I"] = "WVec2I32"
            WVariantSynthProvider.variantTargetTypeMap["Vector3I"] = "WVec3I32"
            WVariantSynthProvider.variantTargetTypeMap["Vector4I"] = "WVec4I32"
            WVariantSynthProvider.variantTargetTypeMap["Vector2U"] = "WVec2U32"
            WVariantSynthProvider.variantTargetTypeMap["Vector3U"] = "WVec3U32"
            WVariantSynthProvider.variantTargetTypeMap["Vector4U"] = "WVec4U32"
            WVariantSynthProvider.variantTargetTypeMap["Quaternion"] = "WQuat"
            WVariantSynthProvider.variantTargetTypeMap["Matrix3"] = "WMat3"
            WVariantSynthProvider.variantTargetTypeMap["Matrix4"] = "WMat4"
            WVariantSynthProvider.variantTargetTypeMap["Transform"] = "WTransform"
            WVariantSynthProvider.variantTargetTypeMap["String"] = "WString"
            WVariantSynthProvider.variantTargetTypeMap["StringView"] = "WStringView"
            WVariantSynthProvider.variantTargetTypeMap["DataBuffer"] = "WDataBuffer"
            WVariantSynthProvider.variantTargetTypeMap["Time"] = "WTime"
            WVariantSynthProvider.variantTargetTypeMap["Uuid"] = "WUuid"
            WVariantSynthProvider.variantTargetTypeMap["Angle"] = "WAngle"
            WVariantSynthProvider.variantTargetTypeMap["ColorGamma"] = "WColorGammaUB"
            WVariantSynthProvider.variantTargetTypeMap["HashedString"] = "WHashedString"
            WVariantSynthProvider.variantTargetTypeMap["TempHashedString"] = "WTempHashedString"
            WVariantSynthProvider.variantTargetTypeMap["VariantArray"] = "WVariantArray"
            WVariantSynthProvider.variantTargetTypeMap["VariantDictionary"] = "WVariantDictionary"
            WVariantSynthProvider.variantTargetTypeMap["TypedPointer"] = "WTypedPointer"
    ...
    def get_child_at_index(self, index):
        ...
        try:
            if index == 0:
                ...
                return self.valobj.CreateValueFromExpression('Type', f"(WVariantType::Enum) {typeValue}")
            ...
            if index == 1:
                ...
                if typeValue in WVariantSynthProvider.variantMap:
                    typeName = WVariantSynthProvider.variantMap[typeValue]
                    if typeName in WVariantSynthProvider.variantTargetTypeMap:
                        targetType = WVariantSynthProvider.variantTargetTypeMap[typeName]
                        expr = f"*reinterpret_cast<{targetType}*>({address})"
                        return self.valobj.CreateValueFromExpression('Value', expr)

                if 'Invalid' == WVariantSynthProvider.variantMap[typeValue]:
                    return self.valobj.CreateValueFromExpression('Invalid', 'nullptr')

                if 'TypedObject' == WVariantSynthProvider.variantMap[typeValue]:
                    ...
                    typeNameStr = str(typeName.GetSummary()).replace('"', '') # A bit hacky, it depends on the WStringView summary provider
            ...
            logger >> "WVariantSynthProvider.get_child_at_index: " + str(inst)
            return None

def WVariant_SummaryProvider(valobj, dict):
    logger = lldb.formatters.Logger.Logger()
    try:
        ...
        logger >> "WVariant_SummaryProvider: " + str(inst)
        return "<error>"
