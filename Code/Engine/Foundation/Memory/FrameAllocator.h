...
class W_FOUNDATION_DLL WDoubleBufferedLinearAllocator
{
public:
...
#endif
  using LinearAllocatorType = WLinearAllocator<WAllocatorTrackingMode::Basics, OverwriteMemoryOnReset>;

  WDoubleBufferedLinearAllocator(WStringView sName, WAllocator* pParent);
  ~WDoubleBufferedLinearAllocator();

  W_ALWAYS_INLINE WAllocator* GetCurrentAllocator() const { return m_pCurrentAllocator; }
...
///
/// This is a convenience wrapper around WDoubleBufferedLinearAllocator that provides a global
...
class W_FOUNDATION_DLL WFrameAllocator
{
public:
  ...
  W_ALWAYS_INLINE static WAllocator* GetCurrentAllocator() { return s_pAllocator->GetCurrentAllocator(); }
...
private:
  W_MAKE_SUBSYSTEM_STARTUP_FRIEND(Foundation, FrameAllocator);

  static void Startup();
  static void Shutdown();

  static WDoubleBufferedLinearAllocator* s_pAllocator;
};
