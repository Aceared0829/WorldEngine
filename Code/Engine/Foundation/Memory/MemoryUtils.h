...
public:
  ...
  /// Constructs \a uiCount objects of type T in a raw buffer at \a pDestination from an existing array of objects at \a pSource by
  ...
  /// Constructs an object of type T in a raw buffer at \a pDestination, by using move construction from \a source.
  template <typename T>
  ...
  /// This function will either move call MoveConstruct or CopyConstruct for a single element \a source, depending on whether it was
  ...
  /// Destructs \a uiCount objects of type T at \a pDestination.
  template <typename T>
  ...
  /// Copies objects of type T from \a pSource to \a pDestination.
  ///
  ...
  /// Copies exactly \a uiNumBytesToCopy from \a pSource to \a pDestination, independent of the involved types and their sizes.
  ...
  /// Moves objects of type T from \a pSource to \a pDestination.
  template <typename T>
  ...
  /// Moves \a uiCount objects in \a pDestination by one object and copies \a source to the free space.
  template <typename T>
  static void Prepend(T* pDestination, const T& source, size_t uiCount);

  /// Moves \a uiCount objects in \a pDestination by one object and moves \a source to the free space.
  template <typename T>
  static void Prepend(T* pDestination, T&& source, size_t uiCount);

  /// Moves \a uiCount objects in \a pDestination by \a uiSourceCount objects and copies \a source to the free space.
  template <typename T>
  static void Prepend(T* pDestination, const T* pSource, size_t uiSourceCount, size_t uiCount);

  /// Tests if objects of type T from \a pSource and \a pDestination are equal.
  template <typename T>
  ...
  /// Aligns the pointer \a ptr by moving its address backwards to the previous multiple of \a uiAlignment.
  template <typename T>
  static T* AlignBackwards(T* pPtr, size_t uiAlignment); // [tested]

  /// Aligns the pointer \a ptr by moving its address forwards to the next multiple of \a uiAlignment.
  template <typename T>
  static T* AlignForwards(T* pPtr, size_t uiAlignment); // [tested]

  /// Aligns the given size \a uiSize by rounding up to the next multiple of the size.
  template <typename T>
  static T AlignSize(T uiSize, T uiAlignment); // [tested]

  /// Checks whether \a ptr is aligned to a memory address that is a multiple of \a uiAlignment.
  template <typename T>
