...
void WSystemInformation::Initialize()
{
  ...
  s_SystemInformation.m_uiCPUCoreCount = static_cast<WUInt32>(sysconf(_SC_NPROCESSORS_ONLN));

  WUInt64 uiPageSize = sysconf(_SC_PAGE_SIZE);

  s_SystemInformation.m_uiMemoryPageSize = static_cast<WUInt32>(uiPageSize);
...
WUInt64 WSystemInformation::GetAvailableMainMemory() const
{
  ...
  return static_cast<WUInt64>(s_SystemInformation.m_uiMemoryPageSize) * static_cast<WUInt64>(vmt.t_free);
}
