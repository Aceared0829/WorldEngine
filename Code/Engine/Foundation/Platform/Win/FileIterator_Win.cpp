...
void WFileSystemIterator::StartSearch(WStringView sSearchStart, WBitflags<WFileSystemIteratorFlags> flags /*= WFileSystemIteratorFlags::All*/)
{
  W_ASSERT_DEV(m_Data.m_Handles.IsEmpty(), "Cannot start another search.");

  m_sSearchTerm = sSearchStart;

  WStringBuilder sSearch = sSearchStart;
  ...
  W_ASSERT_DEV(flags.IsSet(WFileSystemIteratorFlags::Recursive) == false || bHasWildcard == false, "Recursive file iteration does not support wildcards. Either don't use recursion, or filter the filenames manually.");

  if (!bHasWildcard && WOSFile::ExistsDirectory(sSearch))
  {
  ...
  W_ASSERT_DEV(sSearch.IsAbsolutePath(), "The path '{0}' is not absolute.", m_sCurPath);

  m_Flags = flags;

  WIN32_FIND_DATAW data;
  HANDLE hSearch = FindFirstFileW(WDosDevicePath(sSearch), &data);

  if ((hSearch == nullptr) || (hSearch == INVALID_HANDLE_VALUE))
    return;

  m_CurFile.m_uiFileSize = WMath::MakeUInt64(data.nFileSizeHigh, data.nFileSizeLow);
  ...
  m_CurFile.m_LastModificationTime = WTimestamp::MakeFromInt(FileTimeToEpoch(data.ftLastWriteTime), WSIUnitOfTime::Microsecond);
...
{
  constexpr WInt32 ReturnFailure = 0;
  constexpr WInt32 ReturnSuccess = 1;
  constexpr WInt32 ReturnCallInternalNext = 2;
  ...
  if (m_Flags.IsSet(WFileSystemIteratorFlags::Recursive) && m_CurFile.m_bIsDirectory && (m_CurFile.m_sName != "..") && (m_CurFile.m_sName != "."))
  {
    ...
    WStringBuilder sNewSearch = m_sCurPath;
    sNewSearch.AppendPath("*");

    WIN32_FIND_DATAW data;
    HANDLE hSearch = FindFirstFileW(WDosDevicePath(sNewSearch), &data);
    ...
    if ((hSearch != nullptr) && (hSearch != INVALID_HANDLE_VALUE))
    {
      ...
      m_CurFile.m_bIsDirectory = (data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) != 0;
      m_CurFile.m_sParentPath = m_sCurPath;
      W_ASSERT_DEBUG(!m_CurFile.m_sParentPath.EndsWith("/") && !m_CurFile.m_sParentPath.EndsWith("\\"), "Unexpected path separator.");
      m_CurFile.m_sName = data.cFileName;
      m_CurFile.m_LastModificationTime = WTimestamp::MakeFromInt(FileTimeToEpoch(data.ftLastWriteTime), WSIUnitOfTime::Microsecond);
