...
WStringView BuildString(char* szTmp, WUInt32 uiLength, const WArgErrorCode& arg)
{
        ...
        MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US), (LPWSTR)&lpMsgBuf, 0, nullptr) == 0)
  {
    ...
    WStringUtils::snprintf(szTmp, uiLength, "%u (FormatMessageW failed with error code %u)", arg.m_ErrorCode, err);
  ...
  WStringUtils::snprintf(FullMessage, W_ARRAY_SIZE(FullMessage), "%u (\"%s\")", arg.m_ErrorCode, WStringUtf8((LPWSTR)lpMsgBuf).GetData());
  LocalFree(lpMsgBuf);
  return WStringView(FullMessage);
}
