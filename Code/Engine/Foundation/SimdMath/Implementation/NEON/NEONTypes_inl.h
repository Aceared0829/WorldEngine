#pragma once

#include <Foundation/Math/Vec4.h>

#include <arm_neon.h>

#if W_ENABLED(W_COMPILE_FOR_DEBUG)
#  define W_CHECK_SIMD_ALIGNMENT(x) W_CHECK_ALIGNMENT(x, 16)
#else
#  define W_CHECK_SIMD_ALIGNMENT(x)
#endif

namespace WInternal
{
  struct QuadDouble
  {
    float64x2_t xy;
    float64x2_t zw;
  };

  using QuadFloat = float32x4_t;
  using QuadBool = uint32x4_t;
  
  struct QuadBoolWide
  {
    uint64x2_t xy;
    uint64x2_t zw;
  };

  using QuadInt = int32x4_t;
  using QuadUInt = uint32x4_t;

  // Neon equivalent of _mm_movemask_ps
  W_ALWAYS_INLINE uint32_t NeonMoveMask(uint32x4_t x)
  {
    // Isolate the sign bit of each vector element and shift it into its position in the final mask, the horizontally add to combine each element mask.
    alignas(16) static const int32_t shift[4] = {0, 1, 2, 3};
    return vaddvq_u32(vshlq_u32(vshrq_n_u32(x, 31), vld1q_s32(shift)));
  }

} // namespace WInternal

// Converts an WSwizzle into the mask selection format of __builtin_shufflevector
#define W_TO_SHUFFLE(s) (s >> 12) & 3, (s >> 8) & 3, ((s >> 4) & 0x3) + 4, (s & 3) + 4
