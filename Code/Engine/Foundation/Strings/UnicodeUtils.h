...
class W_FOUNDATION_DLL WUnicodeUtils
{
public:
...
public:
  ...
  static WUInt32 GetUtf8SequenceLength(char iFirstByte); // [tested]

  /// Converts the UTF-8 character that starts at pFirstChar into a UTF-32 character.
  static WUInt32 ConvertUtf8ToUtf32(const char* pFirstChar); // [tested]

  /// Computes how many bytes the character would require, if encoded in UTF-8.
  static WUInt32 GetSizeForCharacterInUtf8(WUInt32 uiCharacter); // [tested]
  ...
  static WResult MoveToNextUtf8(const char*& ref_szUtf8, WUInt32 uiNumCharacters = 1); // [tested]

  /// Moves the given string pointer ahead to the next Utf8 character sequence.
  ///
  /// The string may point to an invalid position (in between a character sequence).
  /// It may not point to a zero terminator already.
  static WResult MoveToNextUtf8(const char*& ref_szUtf8, const char* szUtf8End, WUInt32 uiNumCharacters = 1); // [tested]
  ...
  static WResult MoveToPriorUtf8(const char*& ref_szUtf8, const char* szUtf8Start, WUInt32 uiNumCharacters = 1); // [tested]
  ...
  /// If the given string starts with a Utf16 little endian Bom, the pointer is incremented behind the Bom, and the function returns
  /// true.
  ///
  /// Otherwise the pointer is unchanged and false is returned.
  static bool SkipUtf16BomLE(const WUInt16*& ref_pUtf16); // [tested]

  /// If the given string starts with a Utf16 big endian Bom, the pointer is incremented behind the Bom, and the function returns
  /// true.
  ///
  /// Otherwise the pointer is unchanged and false is returned.
  static bool SkipUtf16BomBE(const WUInt16*& ref_pUtf16); // [tested]
  ...
  static WUInt32 DecodeUtf8ToUtf32(ByteIterator& ref_szUtf8Iterator); // [tested]
  ...
  template <typename UInt16Iterator>
  static WUInt32 DecodeUtf16ToUtf32(UInt16Iterator& ref_szUtf16Iterator); // [tested]
  ...
  static WUInt32 DecodeWCharToUtf32(WCharIterator& ref_szWCharIterator); // [tested]

  /// Encodes the given Utf32 character to Utf8 and writes as many bytes to the output iterator, as necessary.
  template <typename ByteIterator>
  static void EncodeUtf32ToUtf8(WUInt32 uiUtf32, ByteIterator& ref_szUtf8Output); // [tested]

  /// Encodes the given Utf32 character to Utf16 and writes as many bytes to the output iterator, as necessary.
  template <typename UInt16Iterator>
  static void EncodeUtf32ToUtf16(WUInt32 uiUtf32, UInt16Iterator& ref_szUtf16Output); // [tested]

  /// Encodes the given Utf32 character to wchar_t and writes as many bytes to the output iterator, as necessary.
  template <typename WCharIterator>
  static void EncodeUtf32ToWChar(WUInt32 uiUtf32, WCharIterator& ref_szWCharOutput); // [tested]
  ...
  struct UtfInserter
  {
    ...
    W_ALWAYS_INLINE UtfInserter(Container* pContainer) { m_pContainer = pContainer; }
    W_ALWAYS_INLINE void operator++() {}
    W_ALWAYS_INLINE UtfInserter& operator++(int) { return *this; }
    W_ALWAYS_INLINE void operator=(IntType rhs) { m_pContainer->PushBack(rhs); }
    W_ALWAYS_INLINE UtfInserter& operator*() { return *this; }

    Container* m_pContainer;
  };

  /// Checks an array of char's, whether it is a valid Utf8 string. If not, it repairs the string, ie by either re-encoding characters or removing them.
  /// Writes the result to the desired container type (WString or WStringBuilder).
  ///
