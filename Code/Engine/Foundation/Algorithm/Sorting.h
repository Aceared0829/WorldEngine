...
public:
  ...
  static void QuickSort(WArrayPtr<T>& inout_arrayPtr, const Comparer& comparer = Comparer()); // [tested]
  ...
  /// See the container overload for detailed performance characteristics.
  template <typename T, typename Comparer>
  static void InsertionSort(WArrayPtr<T>& inout_arrayPtr, const Comparer& comparer = Comparer()); // [tested]
...
private:
  ...
  W_ALWAYS_INLINE constexpr static auto DoCompare(const Comparer& comparer, const Element& a, const Element& b, int) -> decltype(comparer.Less(a, b))
  {
    return comparer.Less(a, b);
  }
  template <typename Element, typename Comparer>
  W_ALWAYS_INLINE constexpr static auto DoCompare(const Comparer& comparer, const Element& a, const Element& b, long) -> decltype(comparer(a, b))
  {
    return comparer(a, b);
  }
  template <typename Element, typename Comparer>
  W_ALWAYS_INLINE constexpr static bool DoCompare(const Comparer& comparer, const Element& a, const Element& b)
  {
  ...
  template <typename Container, typename Comparer>
  static void QuickSort(Container& inout_container, WUInt32 uiStartIndex, WUInt32 uiEndIndex, const Comparer& comparer);

  template <typename Container, typename Comparer>
  static WUInt32 Partition(Container& inout_container, WUInt32 uiLeft, WUInt32 uiRight, const Comparer& comparer);


  template <typename T, typename Comparer>
  static void QuickSort(WArrayPtr<T>& inout_arrayPtr, WUInt32 uiStartIndex, WUInt32 uiEndIndex, const Comparer& comparer);

  template <typename T, typename Comparer>
  static WUInt32 Partition(T* pPtr, WUInt32 uiLeft, WUInt32 uiRight, const Comparer& comparer);


  template <typename Container, typename Comparer>
  static void InsertionSort(Container& container, WUInt32 uiStartIndex, WUInt32 uiEndIndex, const Comparer& comparer);

  template <typename T, typename Comparer>
  static void InsertionSort(WArrayPtr<T>& inout_arrayPtr, WUInt32 uiStartIndex, WUInt32 uiEndIndex, const Comparer& comparer);
};
