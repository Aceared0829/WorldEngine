...
{
  using TokenStream = WHybridArray<const WToken*, 32>;
  ...
  W_FOUNDATION_DLL void SkipWhitespace(const TokenStream& tokens, WUInt32& ref_uiCurToken); // [tested]

  /// Moves ref_uiCurToken forward as long as it is a whitespace token (Whitespace, BlockComment, LineComment) or Newline.
  /// \param tokens Token input.
  /// \param ref_uiCurToken Current location inside 'tokens'. On return, will either stay the same or move forward.
  W_FOUNDATION_DLL void SkipWhitespaceAndNewline(const TokenStream& tokens, WUInt32& ref_uiCurToken); // [tested]
  ...
  W_FOUNDATION_DLL bool IsEndOfLine(const TokenStream& tokens, WUInt32 uiCurToken, bool bIgnoreWhitespace); // [tested]
  ...
  W_FOUNDATION_DLL void CopyRelevantTokens(const TokenStream& source, WUInt32 uiFirstSourceToken, TokenStream& ref_destination, bool bPreserveNewLines); // [tested]
  ...
  W_FOUNDATION_DLL bool Accept(const TokenStream& tokens, WUInt32& ref_uiCurToken, WStringView sToken, WUInt32* pAccepted = nullptr); // [tested]

  /// Overload that matches against a token type instead.
  W_FOUNDATION_DLL bool Accept(const TokenStream& tokens, WUInt32& ref_uiCurToken, WTokenType::Enum type, WUInt32* pAccepted = nullptr); // [tested]
  ...
  W_FOUNDATION_DLL bool Accept(const TokenStream& tokens, WUInt32& ref_uiCurToken, WStringView sToken1, WStringView sToken2, WUInt32* pAccepted = nullptr); // [tested]
  ...
  /// \param pAccepted If not null and true was returned, will be set to the index at which 'sToken1' was matched.
  /// \return Whether the token was found.
  W_FOUNDATION_DLL bool AcceptUnless(const TokenStream& tokens, WUInt32& ref_uiCurToken, WStringView sToken1, WStringView sToken2, WUInt32* pAccepted = nullptr); // [tested]

  /// A token to be matched by the WTokenParseUtils::Accept overload for arrays of tokens.
  /// Can either match a token string or type. If type is WTokenType::Unknown, the token string will be matched.
  struct TokenMatch
  {
    /// This matches a token string of any type.
    TokenMatch(WStringView sToken)
      : m_sToken(sToken)
    {
    }
    /// This matches a token type of any string value.
    TokenMatch(WTokenType::Enum type)
      : m_Type(type)
    {
    }

    /// For internal use. Use one of the other constructors instead.
    TokenMatch(WTokenType::Enum type, WStringView sToken)
      : m_Type(type)
      , m_sToken(sToken)
    {
    }

    WTokenType::Enum m_Type = WTokenType::Unknown;
    WStringView m_sToken;
  };
  ...
  /// \code{.cpp}
  ///   WTokenParseUtils::TokenMatch pattern[] = {"Vec2"_wsv, "("_wsv, WTokenType::Float, ","_wsv, WTokenType::Float, ")"_wsv};
  ///   WHybridArray<WUInt32, 6> matchedTokens;
  ///   W_TEST_BOOL(WTokenParseUtils::Accept(tokens, uiCurToken, pattern, &matchedTokens));
  /// \endcode
  ///
  /// \param tokens Token input.
  ...
  W_FOUNDATION_DLL bool Accept(const TokenStream& tokens, WUInt32& ref_uiCurToken, WArrayPtr<const TokenMatch> matches, WDynamicArray<WUInt32>* pAccepted = nullptr); // [tested]
  ...
  W_FOUNDATION_DLL void CombineTokensToString(const TokenStream& tokens, WUInt32 uiCurToken, WStringBuilder& ref_sResult, bool bKeepComments = true, bool bRemoveRedundantWhitespace = false, bool bInsertLine = false);

  /// Strips out BlockComment, LineComment, EndOfFile and NewLine tokens and combines the rest into a string.
  /// \param tokens The token stream to combine.
  /// \param uiCurToken The start location inside 'tokens' from which point the tokens should be combined.
  /// \param ref_sResult Holds the resulting string after the function call.
  W_FOUNDATION_DLL void CombineRelevantTokensToString(const TokenStream& tokens, WUInt32 uiCurToken, WStringBuilder& ref_sResult);
  ...
  W_FOUNDATION_DLL void CreateCleanTokenStream(const TokenStream& tokens, WUInt32 uiCurToken, TokenStream& ref_destination);

  /// Replaces instances of {PLACEHOLDER} or {PLACEHOLDER[INDEX]} inside a template string with the data appended by a given delegate.
  ///
  ...
  W_FOUNDATION_DLL void RenderTemplate(WStringView sTemplate, const WDelegate<void(WStringView sPlaceholder, WVariant index, bool bOptional, WStringBuilder& ref_sOutput)>& resolveAndAppendPlaceholder, WStringBuilder& out_sOutput); // [tested]
} // namespace WTokenParseUtils
