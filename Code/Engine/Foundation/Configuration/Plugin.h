...
struct WPluginEvent
{
  ...
  WStringView m_sPluginBinary; ///< The file name of the affected plugin.
};

/// Flags for loading a plugin.
struct WPluginLoadFlags
{
  using StorageType = WUInt8;

  enum Enum
  {
    LoadCopy = W_BIT(0),         ///< Don't load a DLL directly, but create a copy of the file and load that instead. This allows to continue working on (and compiling) the DLL in parallel.
    PluginIsOptional = W_BIT(1), ///< When an optional plugin can't be loaded (missing file usually), no error is logged. LoadPlugin() will still return W_FAILURE though.
    CustomDependency = W_BIT(2), ///< The plugin is an injected dependency (usually for the editor), and thus might get treated differently (this is just a tag)
...
using WPluginInitCallback = void (*)();

/// WPlugin manages all dynamically loadable plugins.
///
/// To load a plugin, call WPlugin::LoadPlugin() with the filename of the plugin (without a path).
...
///
/// When a plugin has a dependency on another plugin, it should contain a call to W_PLUGIN_DEPENDENCY() in one of its cpp files.
...
///
/// A plugin may contain one or multiple W_PLUGIN_ON_LOADED() and W_PLUGIN_ON_UNLOADED() functions.
...
/// For anything more complicated, use WStartup instead. Once a plugin is loaded, the startup system will initialize new startup code properly.
class W_FOUNDATION_DLL WPlugin
{
public:
  ...
  static const WCopyOnBroadcastEvent<const WPluginEvent&>& Events(); // [tested]

  /// Calls the W_PLUGIN_ON_LOADED() functions for all code that is already linked into the executable at startup.
  ///
  ...
  /// the W_PLUGIN_ON_LOADED() functions should still be called. The application can decide when the best time is.
  /// Usually a good point in time is right before the app would load the first dynamic plugin.
  /// If this function is never called manually, but WPlugin::LoadPlugin() is called, this function will be called automatically before loading the first actual plugin.
  ...
  static bool ExistsPluginFile(WStringView sPluginFile);

  /// Tries to load a DLL dynamically into the program.
  ///
  /// W_SUCCESS is returned when the DLL is either successfully loaded or has already been loaded before.
  /// W_FAILURE is returned if the DLL cannot be located or it could not be loaded properly.
  ///
  /// See WPluginLoadFlags for additional options.
  static WResult LoadPlugin(WStringView sPluginFile, WBitflags<WPluginLoadFlags> flags = WPluginLoadFlags::Default); // [tested]

  /// Unloads all previously loaded plugins in the reverse order in which they were loaded.
  ///
  /// Also calls W_PLUGIN_ON_UNLOADED() of all statically linked code.
  ...
  static void SetMaxParallelInstances(WUInt32 uiMaxParallelInstances);

  /// \internal struct used by WPlugin macros
  struct W_FOUNDATION_DLL Init
  {
    Init(WPluginInitCallback onLoadOrUnloadCB, bool bOnLoad);
    Init(const char* szAddPluginDependency);
  };

  /// Contains basic information about a loaded plugin.
  struct W_FOUNDATION_DLL PluginInfo
  {
    WString m_sName;
    WHybridArray<WString, 2> m_sDependencies;
    WBitflags<WPluginLoadFlags> m_LoadFlags;
  };

  /// Returns information about all currently loaded plugins.
  static void GetAllPluginInfos(WDynamicArray<PluginInfo>& ref_infos);

  /// \internal Determines the plugin paths.
  static void GetPluginPaths(WStringView sPluginName, WStringBuilder& ref_sOriginalFile, WStringBuilder& ref_sCopiedFile, WUInt8 uiFileCopyNumber);
...
/// W_PLUGIN_DEPENDENCY(Plugin1);
///
/// That instructs the WPlugin system to make sure that Plugin1 gets loaded and initialized before Plugin2 is initialized.
#define W_PLUGIN_DEPENDENCY(PluginName) \
  WPlugin::Init W_PP_CONCAT(W_PP_CONCAT(plugin_dep_, PluginName), W_SOURCE_LINE)(W_PP_STRINGIFY(PluginName))

/// Creates a function that is executed when the plugin gets loaded.
///
/// Just insert W_PLUGIN_ON_LOADED() { /* function body */ } into a CPP file of a plugin to add a function that is called
/// right after the plugin got loaded.
/// If the plugin has depenencies (set via W_PLUGIN_DEPENDENCY()), it is guaranteed that all ON_LOADED functions of the
...
#define W_PLUGIN_ON_LOADED()                                \
  static void plugin_OnLoaded();                             \
  WPlugin::Init plugin_OnLoadedInit(plugin_OnLoaded, true); \
  static void plugin_OnLoaded()
...
///
/// Just insert W_PLUGIN_ON_UNLOADED() { /* function body */ } into a CPP file of a plugin to add a function that is called
...
/// If there are multiple such functions defined within the same DLL, there is no guarantee in which order they are called.
#define W_PLUGIN_ON_UNLOADED()                                   \
  static void plugin_OnUnloaded();                                \
  WPlugin::Init plugin_OnUnloadedInit(plugin_OnUnloaded, false); \
  static void plugin_OnUnloaded()
