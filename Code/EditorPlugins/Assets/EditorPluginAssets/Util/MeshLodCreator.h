...
/// \see WMeshLodCreator::CreateMeshLods()
struct WMeshLodOptions
{
  /// How many LOD assets to create, starting at LOD-1. LOD 0 is the mesh asset itself, which is not
  /// touched. Clamped to WMeshLodCreator::s_uiMaxLods.
  WUInt32 m_uiLodCount = 2;
...
struct W_EDITORPLUGINASSETS_DLL WMeshLodSource
{
  ...
  WUInt8 m_uiBaseSimplification = 0;
  ...
  WDynamicArray<WVariantDictionary> m_MaterialSlots;
  ...
  WDynamicArray<WUuid> m_ExistingLods;

  /// Whether LOD-N already exists. uiLod is 1 based.
  bool HasLod(WUInt32 uiLod) const;
};
...
/// same way WMeshAssetDocumentGenerator does it for a model that is imported with LODs enabled.
///
/// Names and locations match what WMeshPrefabCreator looks for, so that a prefab created from the
/// mesh afterwards finds the LODs and builds an WLodMeshComponent.
class W_EDITORPLUGINASSETS_DLL WMeshLodCreator
{
public:
  /// The most LODs the mesh import generates, and therefore the most the prefab tool looks for.
  static constexpr WUInt32 s_uiMaxLods = 4;
  ...
  static WResult GatherMeshLodSource(const WUuid& meshAssetGuid, WMeshLodSource& out_source);

  /// Creates and saves the LOD documents.
  ///
  /// Existing LOD assets are left alone unless WMeshLodOptions::m_bOverwriteExisting is set.
  static WStatus CreateMeshLods(const WMeshLodSource& source, const WMeshLodOptions& options, WUInt32& out_uiCreated, WUInt32& out_uiSkipped);
  ...
  static WStatus CreateMeshLodsForAll(WArrayPtr<const WUuid> meshAssetGuids, const WMeshLodOptions& options, WUInt32& out_uiCreated, WUInt32& out_uiSkipped);
  ...
  static WUInt8 GetLodSimplification(WUInt8 uiBaseSimplification, WUInt32 uiLod);

  /// The tolerated error for LOD uiLod (1 based), as a percentage. Grows with the level. Mirrors
  /// what the mesh import uses.
  static WUInt8 GetLodSimplificationError(WUInt32 uiLod);

  /// The absolute path of LOD uiLod (1 based) for that source.
  static WString GetLodPath(const WMeshLodSource& source, WUInt32 uiLod);
  ...
  static void GetLodFolderCandidates(WStringView sMeshAssetPath, WStringView sMeshFile, WStringView sMeshIncludeTags, WDynamicArray<WString>& out_folders);
};
