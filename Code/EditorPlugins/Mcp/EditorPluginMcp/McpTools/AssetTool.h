...
class WMcpAssetTool : public WMcpToolProvider
{
  W_ADD_DYNAMIC_REFLECTION(WMcpAssetTool, WMcpToolProvider);
...
private:
  ...
  void ProcessorProgressEventHandler(const WAssetProcessorProgressEvent& e);
  void RecordHistory(const WUuid& guid, WStringView sPath, const WTransformStatus& status, WTime duration, bool bBackground);

  void ExecuteListTypes(const WVariantDictionary& arguments, WMcpToolResult& out_result);
  void ExecuteFind(const WVariantDictionary& arguments, WMcpToolResult& out_result);
  void ExecuteInfo(const WVariantDictionary& arguments, WMcpToolResult& out_result);
  void ExecuteUses(const WVariantDictionary& arguments, WMcpToolResult& out_result);
  void ExecuteThumbnail(const WVariantDictionary& arguments, WMcpToolResult& out_result);
  void ExecuteTransform(const WVariantDictionary& arguments, WMcpToolResult& out_result);
  void ExecuteHealth(const WVariantDictionary& arguments, WMcpToolResult& out_result);
  void ExecuteProcessor(const WVariantDictionary& arguments, WMcpToolResult& out_result);
  void ExecuteHistory(const WVariantDictionary& arguments, WMcpToolResult& out_result);
  void ExecuteListImporters(const WVariantDictionary& arguments, WMcpToolResult& out_result);
  void ExecuteImport(const WVariantDictionary& arguments, WMcpToolResult& out_result);
  ...
  static void WriteAssetIdentity(WMcpJsonWriter& ref_writer, const WSubAsset& subAsset);
  ...
  static constexpr WUInt32 s_uiMaxHistoryEntries = 500;
  ...
  WDeque<HistoryEntry> m_History;
  WEventSubscriptionID m_ProgressSubscription = 0;
