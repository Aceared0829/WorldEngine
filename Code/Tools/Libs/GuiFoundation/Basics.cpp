...
WQtScopedUpdatesDisabled::WQtScopedUpdatesDisabled(
  QWidget* pWidget1, QWidget* pWidget2, QWidget* pWidget3, QWidget* pWidget4, QWidget* pWidget5, QWidget* pWidget6)
{
  ...
  for (int i = 0; i < W_ARRAY_SIZE(pWidgets); ++i)
  {
...
WQtScopedUpdatesDisabled::~WQtScopedUpdatesDisabled()
{
  for (int i = W_ARRAY_SIZE(m_pWidgets) - 1; i >= 0; --i)
  {
...
WQtScopedBlockSignals::WQtScopedBlockSignals(
  QObject* pObject1, QObject* pObject2, QObject* pObject3, QObject* pObject4, QObject* pObject5, QObject* pObject6)
{
  ...
  for (int i = 0; i < W_ARRAY_SIZE(pObjects); ++i)
  {
...
WQtScopedBlockSignals::~WQtScopedBlockSignals()
{
  for (int i = W_ARRAY_SIZE(m_pObjects) - 1; i >= 0; --i)
  {
