...
/// unattended session (see WQtUiServices::IsUnattended()) no one ever does, so the application hangs for
...
class W_GUIFOUNDATION_DLL WQtDialog : public QDialog
{
  Q_OBJECT

public:
  WQtDialog(QWidget* pParent);

  /// Shows the dialog modally, unless no user is present, in which case it reports itself as suppressed
  /// (see WQtUiServices::ReportSuppressedDialog()) and returns QDialog::Rejected without showing anything.
