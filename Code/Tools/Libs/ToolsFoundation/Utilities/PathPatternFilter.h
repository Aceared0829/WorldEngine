...
struct W_TOOLSFOUNDATION_DLL WPathPattern
{
  enum MatchType : WUInt8
  {
    Exact,
    StartsWith,
    EndsWith,
  ...
  void Configure(const WStringView sText);

  /// Returns true if the given text matches this path pattern.
  bool Matches(const WStringView sText) const;
};

/// A collection of WPathPatterns for include/exclude filtering of file paths.
struct W_TOOLSFOUNDATION_DLL WPathPatternFilter
{
  WDynamicArray<WPathPattern> m_ExcludePatterns;
  WDynamicArray<WPathPattern> m_IncludePatterns;

  /// Reads all patterns from the given file.
  ///
  /// The file is parsed with an WPreprocessor, so may contain #include statements and such.
  ...
  WResult ReadConfigFile(WStringView sFile, const WDynamicArray<WString>& preprocessorDefines);

  /// Adds a pattern as either an include or exclude filter.
  void AddFilter(WStringView sText, bool bIncludeFilter);
  ...
  bool PassesFilters(WStringView sText, WStringBuilder* pMatchingFilter = nullptr) const;
};
