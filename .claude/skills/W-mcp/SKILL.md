---
name: W-mcp
description: Drive a running WEditor or WPlayer/game from an agent over MCP - launch one with a port, then call tools to inspect and change a live project. Use for WorldEngine tasks that need the running app rather than files on disk. Triggers - WEditor, WPlayer, WEditorProcessor, MCP server, editor-mcpport, mcpport, transform asset, asset dependencies, asset browser, open document, edit scene, build a scene, game object, component, prefab, property grid, selection, reflected types, RTTI, CVar, editor action, play the game, screenshot, inject input, press key, step frames, export project, create project, build the C++ plugin.
---

# Driving WEditor and the game over MCP

Two hosts, same protocol, **separate ports and separate tool lists**:

| Host | Started with | Answers about |
| --- | --- | --- |
| **Editor** (`WEditor.exe`) | `-editor-mcpport <n>` | assets, documents, scene contents, reflected types, editor actions, the C++ plugin |
| **Game** (`WPlayer.exe`, or the engine process the editor plays in) | `-mcpport <n>` | what is rendered and simulated: screenshots, input, frames, the clock |
...
```shell
# editor
WEditor.exe -project "Data/Samples/PacMan" -unattended -editor-mcpport 7399
# game, standalone - no editor needed
WPlayer.exe -project <project-folder> -scene <path/to/Scene.WBinScene> -profile Default -mcpport 7401
```
...
`claude mcp add --transport http W-editor http://127.0.0.1:7399/mcp`.
...
1. `asset_find` → path or guid. Its paths start with the data directory name
   (`Testing Chambers/Scenes/DynamicFog.WScene`) - that is the form other tools want.
...
```
addObject   property "Children",   type "WGameObject"          -> child object
addObject   property "Components", type "WPointLightComponent"  -> component on it
...
`rtti_derived_types` of `WComponent` lists component types; `rtti_type_properties` gives property
...
`WEditorProcessor.exe -project <path> -compile` (`-recompile` forces CMake too).
...
```shell
WEditor.exe -createProject "C:/Projects/MyGame" -projectTemplate "Basic FPS" -unattended -editor-mcpport 7399
```
...
`WEditorProcessor.exe` takes the same options and quits when done - better when no editor is wanted
afterwards.
