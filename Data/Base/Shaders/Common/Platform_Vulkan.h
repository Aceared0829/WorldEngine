...
#  define PLATFORM_VULKAN W_ON

#  define BEGIN_PUSH_CONSTANTS(Name) struct W_SHADER_STRUCT W_PP_CONCAT(Name, _PushConstants)
#  define END_PUSH_CONSTANTS(Name) \
    ;                              \
    [[vk::push_constant]] W_PP_CONCAT(Name, _PushConstants) Name;
#  define GET_PUSH_CONSTANT(Name, Constant) Name.Constant

#  define BEGIN_MATERIAL_CONSTANTS struct WMaterialConstants
#  define END_MATERIAL_CONSTANTS \
    ;                            \
    StructuredBuffer<WMaterialConstants> materialData BIND_RESOURCE(0, BG_MATERIAL);
...
#  define W_IMAGE_FORMAT(fmt) [[vk::image_format(fmt)]]
