...
struct DispatchParameters
{
  ...
  float2 InvDepthTextureSize; // Inverse of the texture dimensions for 'DepthTexture' (used to convert from pixel coordinates to UVs)
                              // If 'DepthTextureSampler' is an Unnormalized sampler, then this value can be hard-coded to 1.
...
void WriteScreenSpaceShadow(DispatchParameters inParameters, int3 inGroupID, int inGroupThreadID)
{
  ...
  [unroll] for (i = 0; i < READ_COUNT; i++)
  {
    ...
    depths.x = DepthTexture.SampleLevel(DepthTextureSampler, float3(read_xy * inParameters.InvDepthTextureSize, inParameters.EyeIndex), 0);
    depths.y = DepthTexture.SampleLevel(DepthTextureSampler, float3((read_xy + offset_xy) * inParameters.InvDepthTextureSize, inParameters.EyeIndex), 0);
#else
    int bias = bilinear > 0 ? 1 : -1;
    int2 offset_xy = int2(x_axis_major ? 0 : bias, x_axis_major ? bias : 0);

    depths.x = DepthTexture.SampleLevel(DepthTextureSampler, float3(read_xy * inParameters.InvDepthTextureSize, inParameters.EyeIndex), 0);
    depths.y = DepthTexture.SampleLevel(DepthTextureSampler, float3(read_xy * inParameters.InvDepthTextureSize, inParameters.EyeIndex), 0, offset_xy);
#endif
  ...
  // write the result
  {
    ...
    OutputTexture[int3(write_xy, inParameters.EyeIndex)] = result;
  }
}
