...
// Demonstrates WInputManager::SetMouseCursor() and the WMouseCursorRenderer that draws it.
//
...
// Textures/Cursor_Arrow.WTextureAsset
static constexpr WStringView s_sCursorArrow = "{ d4aca571-6900-48aa-93c6-3d388f551f50 }"_wsv;

void RTSGameState::UpdateMouseCursor()
{
  WMouseCursorDesc cursor;
  ...
  switch (GetActiveGameMode())
  {
    case RtsActiveGameMode::BattleMode:
      ...
      {
        m_CursorAnimation += WClock::GetGlobalClock()->GetTimeDiff();
        cursor.m_Rotation = WAngle::MakeFromDegree((float)m_CursorAnimation.GetSeconds() * 180.0f);
      }
      else
      {
        m_CursorAnimation = WTime::MakeZero();
      }

      // Tint the cursor while something is selected, to show off the color modulation.
      if (m_SelectedUnits.GetCount() > 0)
      {
        cursor.m_Color = WColor::OrangeRed;
      }
      break;
  ...
  WInputManager::SetMouseCursor(cursor);
}
