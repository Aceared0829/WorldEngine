# How to build the WorldEngine custom version of clang-tidy
 ...
 * Copy the W folder to `llvm-project/clang-tools-extra/clang-tidy/W`
 ...
 * Copy the resulting executable from `build/Release/bin/clang-tidy.exe` to `WorldEngine/Data/Tools/Precompiled/clang-tidy/clang-tidy.exe` when done
...
⚠️When running clang-tidy.exe out of the directory it was build into, it will pick up a different set of header files and compile errors might appear. So copy the executable out of the build directory before attempting to run it on the WorldEngine source code. For local minimal test cases the clang-tidy.exe can remain in the build output directory.\
If you need to run and debug on W code, open the clang-tidy VS properties and do the following.
  * Change the paths in the steps below as follows:
    * **C:\Code\llvm-project**: The root directory of your llvm-project clone.
    * **C:\Code\WorldEngine**: The root directory of your W clone.
    * **C:\Code\WorldEngine\Code\Engine\Core\ResourceManager\Implementation\Resource.cpp**: The file you want to check.
  * Build Events -> Post-Build-Event: `copy C:\Code\llvm-project\build\Debug\bin\clang-tidy.exe "C:\Code\WorldEngine\Data\Tools\Precompiled\clang-tidy\clang-tidy.exe" /Y`
  * Debugging:
    * Command: `C:\Code\WorldEngine\Data\Tools\Precompiled\clang-tidy\clang-tidy.exe`
    * Command Arguments: `-p C:\Code\WorldEngine\Workspace\clang-tidy --checks=-*,W-name-check,modernize-use-default-member-init,modernize-use-equals-default,modernize-use-using,clang-analyzer-core.* "--header-filter=^((?!ThirdParty|DirectXTex|ogt_vox|ui_).)*$" --extra-arg=-DBUILDSYSTEM_CLANG_TIDY=1 "--extra-arg=-isystemC:\Code\WorldEngine\llvm\lib\clang\16\include" C:\Code\WorldEngine\Code\Engine\Core\ResourceManager\Implementation\Resource.cpp`
    * Working Directory: `C:\Code\WorldEngine\Data\Tools\Precompiled\clang-tidy`