  ...
  namespace tidy
  {
    namespace W
    {
      ...
      std::string AddPrefixForType(clang::StringRef typeName, std::string newName, bool* prefixAdded = nullptr)
      {
        ...
        else if (typeName == "WSharedPtr" || typeName == "WUniquePtr" ||
                 typeName == "WPointerWithFlags" || typeName == "QPointer" ||
        ...
        else if (typeName == "WStringView" || typeName == "WHybridString" ||
                 typeName == "WHashedString" || typeName == "WTempHashedString" ||
                 typeName == "basic_string" || typeName == "WStringBuilder" ||
                 typeName == "QString")
        {
          if (prefixAdded)
            *prefixAdded = true;
        ...
        else if (typeName == "WAtomicBool")
        {
          if (prefixAdded)
            *prefixAdded = true;
          return newName.insert(0, "b");
        }
        else if (typeName == "WAtomicInteger32" || typeName == "WAtomicInteger64")
        {
          if (prefixAdded)
            *prefixAdded = true;
          return newName.insert(0, "i");
        }
        else if (typeName.startswith("WVec") || typeName.startswith("WSimdVec"))
        {
          if (prefixAdded)
            *prefixAdded = true;
          return newName.insert(0, "v");
        }
        else if (typeName.startswith("WMat") || typeName.startswith("WSimdMat"))
        {
          if (prefixAdded)
            *prefixAdded = true;
          return newName.insert(0, "m");
        }
        else if (typeName.startswith("WQuat") || typeName == "WSimdQuat" || typeName == "WSimdQuatd")
        {
          if (prefixAdded)
            *prefixAdded = true;
          return newName.insert(0, "q");
        }
        else if (typeName == "WSimdFloat" || typeName == "WSimdDouble")
        {
          if (prefixAdded)
            *prefixAdded = true;
      ...
      std::string AddPrefix(const std::string& baseName, const Type* type, bool* prefixAdded = nullptr)
      {
        ...
        else
        {
          ...
          if (recordDecl)
          {
            ...
            else if (auto declTemplate = dyn_cast<ClassTemplateSpecializationDecl>(recordDecl); declTemplate && (recordName == "atomic" || recordName == "WAtomicInteger"))
            {
          ...
          else
          {
            ...
            else
            {
              ...
              {
                auto typedefName = typedefType->getDecl()->getName();
                if (typedefName == "WUInt8" || typedefName == "WUInt16" ||
                    typedefName == "WUInt32" || typedefName == "WUInt64" ||
                    typedefName == "size_t" || typedefName == "DWORD")
                {
                  return newName.insert(0, "ui");
                }
                else if (typedefName == "WInt8" || typedefName == "WInt16" ||
                         typedefName == "WInt32" || typedefName == "WInt64" ||
...
// m_bIsUsed[3]; WDynmicArray<bool> m_bVar? what about WUInt8 m_uiData[128];
// -> On fixed size arrays keep the already existing prefix
// * static WWorldModuleTypeId TYPE_ID. This is not a constant. Why all upper
