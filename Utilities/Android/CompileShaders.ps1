...
if ([string]::IsNullOrEmpty($BinariesDir)) {
    throw "BinariesDir parameter is required. Set it to the directory containing WShaderCompiler"
}
...
$shaderCompilerPath = "$BinariesDir/WShaderCompiler"
if ($IsWindows) {
    $shaderCompilerPath += ".exe"
}

if (-not (Test-Path $shaderCompilerPath)) {
    throw "WShaderCompiler not found at: $shaderCompilerPath"
}

& $shaderCompilerPath -renderer Vulkan -platform VULKAN -project "$RootDir/Data/Base" -shader Shaders/Pipeline
if ($LASTEXITCODE -ne 0) { throw "WShaderCompiler failed for Data/Base (exit code $LASTEXITCODE)." }

& $shaderCompilerPath -renderer Vulkan -platform VULKAN -project "$RootDir/Data/UnitTests" -shader "RendererTest/Shaders"
if ($LASTEXITCODE -ne 0) { throw "WShaderCompiler failed for Data/UnitTests (exit code $LASTEXITCODE)." }

& $shaderCompilerPath -renderer Vulkan -platform VULKAN -project "$RootDir/Data/Samples/ShaderExplorer" -shader Shaders
if ($LASTEXITCODE -ne 0) { throw "WShaderCompiler failed for Data/Samples/ShaderExplorer (exit code $LASTEXITCODE)." }
